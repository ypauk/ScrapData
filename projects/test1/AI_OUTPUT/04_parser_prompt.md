...
Для проекта следует использовать **существующий **`PlaywrightEngine`** для навигации и получения HTML**, а затем **BeautifulSoup для разбора HTML**.

Причины:

- API в предоставленных материалах не обнаружен; `network.har` пустой.
- Сайт использует JavaScript, хотя значительная часть нужных данных уже присутствует в server-rendered HTML.
- Framework уже фиксирует `PlaywrightEngine` как единственный способ сетевого сбора: `scrape_data(engine) → List[str]`.
- Каталог использует обычную пагинацию `?p=2`, `?p=3` и т. д.
- Детальные поля — description, изображения и характеристики — находятся на product pages, поэтому одних listing pages недостаточно.
- Авторизация для каталога не требуется.
- Cloudflare присутствует, поэтому использование уже настроенного browser context безопаснее попытки реализовать отдельный HTTP-клиент.
Это соответствует исходному prompt и зафиксированным интерфейсам framework.

**Плюсы:**

- совместимо с существующим `main.py`;
- сохраняется browser context с cookies/proxy/user-agent;
- JavaScript выполняется браузером;
- можно последовательно обходить pagination и product pages;
- BeautifulSoup позволяет отделить сетевой слой от extraction logic.
**Минусы:**

- браузер тяжелее обычного HTTP-клиента;
- 429 товаров означают заметное число переходов;
- Cloudflare может блокировать автоматизированные сессии;
- изменение HTML/CSS-селекторов потребует адаптации parser.
`requests`/`httpx` здесь не следует предлагать: framework непосредственно требует `PlaywrightEngine`. Selenium и Scrapy также не дают преимущества при существующей архитектуре.
...
`extract product URLs` — только навигационная логика внутри `scraper.py`
...
`main.py → parse_html_data(raw_pages_content)`
...
Одновременно `scraper.py` при обработке product page получает URL изображений и скачивает сами изображения в директорию, предусмотренную `OUTPUT_DIR`. Это сетевая операция и потому относится к `scraper.py`, а не к `parser.py`.

Ключевой контракт не меняется:

`scrape_data(engine) → List[str]`

`parse_html_data(raw_pages_content) → List[dict]`

`main.py` не изменяется. Framework прямо фиксирует этот интерфейс.

Для текущего теста scraper должен остановиться после получения **2 уникальных product URLs**. Для будущего full scrape ограничение снимается и обходится вся pagination.
...
- открыть стартовый category URL;
- дождаться появления товарных карточек;
- получить HTML listing page;
- определить product URLs;
- определить следующую страницу;
- перейти к product pages;
- получить HTML каждой product page;
- скачать изображения товаров;
- вернуть список HTML product pages.
Для тестового запуска — только 2 товара.

**Возвращаемое значение:**

`List[str]`, где каждый элемент — полный HTML одной product page.

Не следует возвращать словари, `BeautifulSoup`-объекты или уже распарсенные товары.

### _fetch_listing_html(engine, url) → str

**Назначение:** открыть category URL через переданный `PlaywrightEngine`, дождаться контента каталога и вернуть HTML.

**Ожидаемый selector для ожидания:**
...
Источник подтверждает наличие `ul.products-grid...` и `li.item`.

**Алгоритм:**

- открыть URL;
- дождаться товарной сетки;
- при необходимости дождаться `li.item`;
- сделать умеренную паузу через существующий `random_delay()`;
- вернуть HTML страницы.
### _extract_product_urls(html) → List[str]

Эта функция допустима в `scraper.py`, поскольку извлекает **навигационные URL**, а не бизнес-поля товара.

Внутри можно использовать BeautifulSoup только для поиска ссылок, не превращая scraper в бизнес-парсер.

**Основной selector:**

```основной selector:
ul.products-grid.category-products-grid li.item h2.product-name a
```
...
В предоставленном HTML оба варианта ведут на product URL. Например, listing содержит ссылку на `polar-dm071.html` и отдельную ссылку изображения.

После извлечения:

- привести относительные URL к абсолютным;
- убрать дубли;
- сохранить порядок появления;
- для теста взять первые 2 уникальных URL.
**ПРЕДПОЛОЖЕНИЕ:** первые два товара из текущего listing являются подходящими товарами для тестового файла, поскольку отдельный критерий выбора двух товаров в prompt не задан.

### _get_next_page_url(html, current_url) → str | None

Пагинация — обычная URL pagination.
...
а последующие страницы используют `?p=3`, `?p=4` и т. д.
...
- вернуть `None`, если next отсутствует.
Для защиты от циклов желательно вести множество уже посещённых listing URLs внутри `scrape_data()`.

### _fetch_product_html(engine, url) → str

**Назначение:** открыть product page и вернуть её HTML.
...
- дождаться title/product content;
- применить `random_delay()`;
- получить HTML;
- передать HTML дальше без бизнес-парсинга.
### _download_product_images(engine, html, product_url, product_index) → List[str]

Эта вспомогательная функция нужна потому, что клиент требует **не только URL изображений, но и физическое скачивание файлов**.

Она относится к `scraper.py`, поскольку скачивание является сетевой операцией.

Алгоритм:

- из HTML product page получить image URLs;
- нормализовать абсолютные URL;
- удалить дубли;
- скачать каждое изображение через доступный browser/network context;
- сохранить в `OUTPUT_DIR`;
- использовать имя файла из URL, если оно доступно;
- при конфликте имён обеспечить уникальное локальное имя;
- ошибку одной картинки не считать ошибкой всего товара;
- логировать failed download.
**ПРЕДПОЛОЖЕНИЕ:** поскольку предоставленный `exporter.py` менять нельзя и его контракт не описывает отдельный image-export механизм, изображения следует сохранять как побочный результат scraper в существующую output-директорию, а `image_name` формировать независимо от JSON-export.

## 3.2. Алгоритм обхода

### Тестовый режим

- Start URL:`https://www.professionele-koeling.nl/koelkasten-kisten.html`
- Получить listing HTML.
- Извлечь product URLs.
- Взять первые 2 уникальных URL.
- Для каждого:

- открыть product page;
- получить HTML;
- скачать все найденные изображения;
- сохранить HTML в результирующий список.
- Вернуть `List[str]`.
Pagination для теста фактически не нужна, если на первой странице доступны минимум два товара.

### Full scrape

После утверждения теста:

- открыть первую страницу;
- получить product URLs;
- открыть все product pages;
- перейти на `?p=2`;
- повторять до отсутствия `a.next`;
- поддерживать `seen_product_urls`;
- не допускать повторной обработки одного URL.
По предоставленным данным сейчас каталог содержит 429 товаров, по 36 на страницу, то есть приблизительно 12 страниц.

## 3.3. Ожидание и задержки

`random_delay()` брать из `app.utils`, не дублировать её.

Рекомендуемые точки вызова:

- после открытия listing page;
- между переходами на product pages;
- между страницами pagination;
- перед/после последовательных скачиваний изображений при необходимости.
Не делать агрессивных параллельных переходов: наличие `cf_clearance` показывает, что сайт использует Cloudflare-механизм.

Lazy loading по предоставленному HTML не подтверждён. Поэтому дополнительный scroll не нужен, пока live-тест не покажет, что изображения появляются только после scroll.

# 4. Проектирование app/parser.py

`parser.py` **не делает ни одного сетевого запроса**.

Он получает HTML, строит `BeautifulSoup` и извлекает данные.

## 4.1. Интерфейс функций

ФункцияНазначениеВходные параметрыВозвращаемое значение`parse_html_data(html_pages)`Преобразует список product HTML в итоговый список товаров`List[str]``List[dict]``parse_product(html)`Извлекает все поля одного товара`str``dict``_parse_breadcrumbs(soup)`Извлекает Breadcrumb`BeautifulSoup``str``_parse_title(soup)`Извлекает Title`BeautifulSoup``str``_parse_short_description(soup)`Извлекает Short description`BeautifulSoup``str``_parse_images(soup)`Извлекает URL изображений и имена файлов`BeautifulSoup``(imageurl, image_name)``_parse_prices(soup)`Извлекает Price и Sale price`BeautifulSoup``(price, sale_price)``_parse_description(soup)`Извлекает полное описание без HTML`BeautifulSoup``str``_parse_specs(soup)`Извлекает характеристики`BeautifulSoup`структура согласно спецификации`_parse_spec_detail(soup)`Разделяет пары `header: value``BeautifulSoup`структура согласно спецификации# 5. Спецификация полей

Важно: названия ключей должны оставаться **точно такими, как в **`DS-PRK-Scraper.json`. Нельзя исправлять, переименовывать или нормализовать `Title ` и `Description ` в `Title`/`Description`.

Авторитетная спецификация прямо запрещает добавлять, удалять, переименовывать, объединять или разделять output columns.
...
Значение — абсолютный URL товара.
...
В предоставленном product HTML присутствуют:
...
**ПРЕДПОЛОЖЕНИЕ:** поскольку пример DS-PRK-Scraper.json содержит `"Breadcrumb": "Home"`, итоговое значение следует формировать согласно фактическому требованию спецификации, а не автоматически включать всю цепочку. Если ожидается полная breadcrumb chain, это необходимо подтвердить тестом.

Чтобы не нарушить authoritative schema, нельзя самостоятельно переинтерпретировать это поле.
...
HTML необходимо преобразовать в чистый текст:

- `<br>` → пробел/перенос;
- HTML-теги удалить;
- повторяющиеся whitespace нормализовать;
- рекламный HTML не сохранять.
Требование клиента — именно чистый текст, не HTML.

### imageurl

Источник — изображения product page.

**ПРЕДПОЛОЖЕНИЕ:** основной selector следует выбирать по фактической product-page HTML после live-проверки, например изображения внутри product image gallery. Предоставленный compressed HTML показывает структуру listing image, но не предоставляет полный HTML image gallery product page.

Каждый URL хранится отдельно логически, но если authoritative output предусматривает одно поле со значениями, применяется буквально указанная инструкция:

multiple → separated by comma.

Нельзя создавать дополнительное поле вроде `images`.
...
или другого фактического filename.
...
Сами изображения при этом должны быть скачаны scraper-ом.
...
Если присутствует `old-price`, его значение становится обычной ценой.

Цена должна проходить через существующий `app.utils.clean_price()`, а не через новую функцию очистки цены.
...
или соответствующий тип, который уже использует exporter/framework.
...
Если скидочной цены нет — **пустое значение**, не обычная цена.

Для отсутствующего значения использовать `""`, поскольку исходные требования явно говорят «пустое», а не копирование regular price.

### Description

Источник — полный блок описания product page.

Не использовать `short-description`.

Нужно:

- найти основной description container;
- удалить HTML;
- сохранить текстовый порядок;
- сохранить смысловые переносы строк;
- убрать технический HTML;
- не добавлять дополнительные поля.
### Specs

Это наиболее неоднозначное поле исходной спецификации.

В `DS-PRK-Scraper.json` значение выглядит как:

```
https://www.awesomescreenshot.com/... each own colom
```

Одновременно prompt прямо говорит, что это инструкция, а не повреждённые данные.

Поэтому **нельзя подменять его собственной структурой без подтверждения**.

**ПРЕДПОЛОЖЕНИЕ:** `Specs` должен содержать отдельные значения характеристик в соответствии с фактической таблицей/specification block product page, но точный формат представления необходимо валидировать на тестовом JSON.

### Spec_detail

Это поле должно обрабатывать строки вида:
...
Инструкция authoritative specification:
...
Нельзя складывать весь список характеристик в одну строку.

При отсутствии конкретной характеристики соответствующее значение оставляется пустым согласно требованиям клиента.

**ПРЕДПОЛОЖЕНИЕ:** если конечный exporter ожидает фиксированные поля, набор ключей `Spec_detail` должен соответствовать фактическому набору колонок authoritative JSON, а не динамически создаваться по произвольным характеристикам. Это нужно проверить на полном содержимом `DS-PRK-Scraper.json` перед реализацией.

# 6. Финальная структура одного товара

Структура должна сохранять **ровно те ключи и их порядок, которые заданы authoritative **`DS-PRK-Scraper.json`:
...
Это именно схема проектирования, а не утверждение конкретных значений для второго тестового товара.

Нельзя добавлять, например:
...
- `stock`;
- `product_id`.
Даже если эти данные технически доступны на сайте.

# 7. Обработка ошибок

СценарийДействиеTimeout listingПовторить ограниченное число раз, например 2; после неудачи записать ошибку через существующий logger и остановить/пропустить страницуTimeout product pageПовторить до 2 раз; после неудачи залогировать URL и пропустить товарHTTP 403 / Cloudflare blockЗалогировать, прекратить активный обход; не пытаться обходить защитуCAPTCHAЗалогировать и остановить scraping для ручного вмешательстваProduct selector отсутствуетЗалогировать; не считать весь запуск повреждённымЦена отсутствуетВернуть пустое значение; продолжить parsingSale price отсутствует`""`; продолжить parsingImage download failedЗалогировать конкретный image URL; остальные изображения и товар продолжить обрабатыватьDescription отсутствуетВернуть `""`Spec отсутствуетВернуть пустое значениеПустой listingЗалогировать и завершить paginationNext page отсутствуетНормальное завершение paginationDuplicate product URLПропуститьBroken HTMLBeautifulSoup всё равно пытается разобрать документ; отсутствующие поля получают допустимые пустые значенияNetwork interruptionПовторить текущий переход; после исчерпания retry залогировать и остановить/пропустить согласно уровню страницыНе нужно реализовывать отдельный API/JSON error handler: текущая задача работает с HTML.

# 8. Оценка рисков

- **Cloudflare** — существенный риск; присутствует `cf_clearance`.
- **Требуется Login** — нет для каталога/product pages.
- **Rate Limiting** — точный лимит неизвестен; использовать `random_delay()`.
- **CAPTCHA** — не подтверждена, но должна быть обработана как возможный runtime-сценарий.
...
- **Большое количество product pages** — для полного scrape будет существенно больше сетевых переходов, чем число listing pages.
- **Изображения** — отдельные downloads могут завершаться ошибками.
- **Неоднозначность **`Specs`** / **`Spec_detail` — требует проверки тестового результата.
# 9. Что останется неизвестным

Следующие моменты нельзя достоверно определить из предоставленного compressed HTML и поэтому их не следует выдумывать:

- **Точный selector полной image gallery на product page.**
- **Точный HTML-контейнер полного Description.**
- **Точный HTML-контейнер таблицы Specs.**
- **Точный формат конечного **`Specs`**, поскольку пример содержит инструкцию, а не полноценную структуру данных.**
- **Точный формат **`Spec_detail`** в экспортируемом JSON при наличии нескольких динамических характеристик.**
- **Точный смысл **`Breadcrumb`**: только **`"Home"`** или вся цепочка.**
- **Точный механизм скачивания изображений через существующий **`PlaywrightEngine`, поскольку API самого framework для binary download в prompt не приведён.
- **Точный механизм exporter для локально скачанных изображений**, поскольку менять `exporter.py` запрещено.
- **Поведение сайта при фактическом live-запросе из scraper:** Cloudflare может вести себя иначе, чем в сохранённых материалах.
При этом это не означает, что задача блокирована. Основная архитектура и навигационная схема уже определены. Для теста потребуется проверить эти selectors на реальном HTML.

# 10. Порядок реализации

### 1. parser.py

Сначала реализовать parser на сохранённом HTML.

Причина: он полностью детерминирован, не требует сети и может быть проверен на `page.html`/product HTML.
...
- image downloading;
- retry/delay logic.
Так сохраняется разделение ответственности: scraper не знает бизнес-структуру данных, parser не знает сеть.

### 3. Интеграционная проверка с существующим main.py

`main.py` не изменять.

Проверить именно существующие контракты:
...
- порядок полей.
### 5. Только после успешного теста — full scrape

Не использовать старый `checkpoint.processed_count = 49` как источник количества товаров: текущие предоставленные материалы указывают 429 товаров.
...
Scraper отвечает за сеть, navigation, pagination и downloads.

`parser.py`**:**

- `parse_html_data()`
- `parse_product()`
- отдельные небольшие функции extraction для каждого требуемого поля.
Parser работает исключительно с уже полученным HTML.

**Итоговая структура:** строго структура `DS-PRK-Scraper.json`, без добавления новых полей и без переименования существующих.

**Главные риски:** Cloudflare, rate limiting, скачивание изображений, изменение HTML и неоднозначность точного формата `Specs`/`Spec_detail`.

**Для текущего этапа:** реализовывать только тестовый сбор двух товаров; полный обход 429 товаров выполнять после проверки тестового JSON. Исходный prompt прямо фиксирует, что код сейчас писать не требуется — требуется архитектурное проектирование.
