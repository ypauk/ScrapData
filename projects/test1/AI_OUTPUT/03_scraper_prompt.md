...
# Проектирование scraper.py и parser.py — Professionele-Koeling.nl

## Почему выбран HTML + Playwright

Выбирается **HTML-сбор через существующий PlaywrightEngine**.

API/GraphQL использовать не следует: в предоставленных материалах `network.har` пустой, а `notes.txt` прямо указывает, что API нет. При этом нужные данные находятся непосредственно в HTML листинга и product pages. Актуальная страница категории также отдаёт названия, ссылки и цены товаров в HTML.

Playwright обязателен не потому, что данные невозможно получить обычным HTTP, а потому что это зафиксировано архитектурой проекта: `main.py` передаёт в `scrape_data()` готовый `PlaywrightEngine`, а `browser.py` уже отвечает за браузерный контекст, cookies, proxy и user-agent.

### Плюсы

- соответствует существующему framework;
- не требует нового HTTP-слоя;
- позволяет использовать уже существующие cookies/Cloudflare-сессию;
- product pages доступны через обычную навигацию;
- не требуется API или отдельная инфраструктура;
- HTML можно передавать в `parser.py` без сетевой зависимости.
### Минусы

- Playwright тяжелее обычного HTTP-клиента;
- возможны проблемы с Cloudflare;
- cookies могут истечь;
- HTML-селекторы могут измениться;
- полный scrape 429 товаров потребует много переходов.
# 1. Полный поток данных

Для текущего тестового запуска нужны только первые 2 товара.
...
Ключевое решение: **для **`parse_html_data()`** возвращать только HTML product pages, а не смешивать listing HTML и product HTML**.

Это упрощает контракт парсера: каждый элемент `raw_pages_content` является страницей товара.

`parse_listing()` при этом остаётся отдельной функцией для обработки HTML листинга, поскольку такой интерфейс уже предусмотрен framework.
...
Главная и обязательная функция.

**Назначение:**

Организует весь сетевой обход:

- открыть категорию;
- дождаться появления product cards;
- получить URL товаров;
- выбрать первые два уникальных URL для тестового запуска;
- открыть каждую product page;
- получить её HTML;
- вернуть список HTML.
**Вход:**

`engine` — готовый экземпляр `PlaywrightEngine`.
...
Для будущего полного scrape ограничение `2` должно быть заменено на условие полного обхода категории.

### _fetch_listing_html(engine, url) → str

**Назначение:**

Открывает URL категории через существующий PlaywrightEngine и возвращает HTML после загрузки нужного контента.
...
На актуальной странице присутствует блок товаров и пагинация.

### Ожидание контента

Ожидать следует наличие карточек:
...
Не следует ждать конкретный товар по имени.

### _fetch_product_html(engine, url) → str

**Назначение:**

Открывает product URL и возвращает полный HTML страницы товара.
...
На основании предоставленного `product-page.html`:

- `.breadcrumbs`
- `.product-name`
- `.short-description`
- `.price-box`
Для описания характеристик и изображений нужно использовать соответствующие блоки product page, но их точная разметка **не представлена в предоставленном сокращённом HTML**.

Поэтому конкретные селекторы этих двух частей нельзя выдавать как установленные факты.

### _extract_product_urls(html) → List[str]

**Назначение:**

Из HTML листинга извлекает URL product pages.

Это является навигационной логикой scraper, поэтому такая функция допустима в `scraper.py`.

**Вход:**

HTML листинговой страницы.

**Выход:**

список уникальных product URLs.
...
Альтернативная ссылка из той же карточки:

```
li.item h2.product-name a[href]
```

Предпочтительно брать ссылку из `h2.product-name a`, либо сверять оба источника и дедуплицировать URL.

В предоставленном HTML карточка `Polar DM071` имеет product URL одновременно у `.product-image` и `.product-name`.
...
Определяет следующую страницу категории.
...
```
div.pager div.pages ol li a[href]
```

В предоставленном HTML следующая страница имеет вид:
...
- если URL уже посещался — вернуть `None`.
## 2.2. Алгоритм обхода

### Текущий тест — 2 товара
...
- берёт первые **2 уникальных URL**;
- открывает product page №1;
- получает HTML;
- делает паузу;
- открывает product page №2;
- получает HTML;
- возвращает два HTML.
При этом scraper **не должен ограничиваться только товарами, которые имеют кнопку **`In winkelwagen`. Клиент явно требует включать товары без наличия.

### Полный scrape после approval

Алгоритм меняется только в части лимита:
...
while page exists:
    ...
    add new URLs
    if reached requested/full amount:
        stop

    next_page = _get_next_page_url(...)
...
Пагинация должна строиться по фактическому `next` URL, а не по заранее рассчитанному количеству страниц.

Это особенно важно, потому что старый `checkpoint.json` нельзя использовать как источник актуального количества товаров.

## Ожидания, скролл и клики

### Скролл

**Не требуется как часть основной стратегии.**

В предоставленных материалах infinite scroll не обнаружен, а товары уже присутствуют в HTML.

### Lazy loading

Для product images потенциально может использоваться lazy loading, но конкретный механизм не предоставлен.

Если изображения загружаются только после появления элемента в viewport, scraper может перед получением HTML выполнить обычный scroll страницы вниз/вверх. Это является допустимой сетевой/браузерной операцией.

Однако это должно быть реализовано **только если фактический DOM подтверждает lazy loading**.

**ПРЕДПОЛОЖЕНИЕ:** изображения могут иметь lazy-loading атрибуты, но по имеющимся материалам это не подтверждено.

### Клики

Кнопки `Add to cart`, wishlist, compare и т. п. нажимать не нужно.

Табы раскрывать только если обязательные характеристики реально скрыты внутри collapsed/tabbed UI. В предоставленных материалах это не подтверждено.

## random_delay()

Использовать существующую `app.utils.random_delay()`:

- между открытием разных product pages;
- между переходами по страницам категории;
- при необходимости перед повторным запросом после timeout.
Не создавать собственную функцию задержек.

Задержка должна быть частью scraper, поскольку это сетевое поведение.

# 3. Проектирование app/parser.py

Парсер полностью offline: он получает HTML и не делает никаких сетевых запросов.

## 3.1. Интерфейс функций

ФункцияНазначениеВходВозврат`parse_listing(html)`Разобрать HTML категории и извлечь карточки товаров`html: str``List[dict]``parse_single_item(card)`Разобрать одну карточку листинга`bs4.element.Tag``dict``parse_html_data(raw_pages_content)`Обработать список HTML product pages`List[str]``List[dict]``parse_product(html)`Извлечь все обязательные поля одной product page`str``dict``_clean_text(value)`Удалить HTML и нормализовать whitespace`str/Tag/None``str``_parse_price(value)`Передать значение цены в существующую логику очистки цены`str/None`нормализованная цена`_parse_specs(container)`Разобрать характеристики`Tag/None`структура `Specs`/`Spec_detail``_extract_image_urls(container)`Получить URL изображений`Tag/None``List[str]``_image_name_from_url(url)`Получить имя файла из URL`str``str`Последние вспомогательные функции являются внутренними и не являются частью публичного интерфейса.

# 3.2. parse_listing(html)

### Назначение

Используется для разбора HTML категории.

### Основной селектор карточек

```
ul.products-grid li.item
```

На предоставленной странице карточки действительно находятся в `li.item`.
...
Для текущего scrape эта функция не должна быть источником детальных product data: она нужна прежде всего для анализа листинга и может использоваться для тестирования структуры.

# 3.3. parse_single_item(card)

Для карточки доступны:
...
```listing image url
a.product-image img[src]
```

Но listing image — это thumbnail/cache URL. Поскольку клиент требует скачивать изображения и окончательная логика должна использовать product images, **не следует считать thumbnail из листинга окончательным **`imageurl`** без проверки product page**.
...
Это соответствует явному требованию клиента.

# 3.4. parse_product(html)

Это основная функция извлечения данных.

Она должна создавать **только** поля из `DS-PRK-Scraper.json`, без дополнительных полей.
...
- Specs
- Spec_detail
Важно: `Title ` и `Description ` должны сохранять пробел в конце имени ключа, поскольку именно такие имена находятся в authoritative `DS-PRK-Scraper.json`.

# 3.5. Спецификация полей
...
Если `parse_product()` получает только HTML без URL отдельно, URL необходимо брать из `<link rel="canonical">`, если такой элемент присутствует.

**ПРЕДПОЛОЖЕНИЕ:** canonical URL присутствует на product page.

Если canonical отсутствует, scraper должен сохранять URL вместе с HTML только если существующий контракт `main.py` это допускает. Поскольку контракт сейчас передаёт только `List[str]`, добавлять новый тип данных в scraper нельзя.

Поэтому наличие URL внутри HTML является необходимым условием для полностью надёжного заполнения этого поля.
...
В `DS-PRK-Scraper.json` пример содержит:

```
"Breadcrumb": "Home"
```

Поэтому **нельзя самостоятельно превращать его в полный breadcrumb**, если specification требует именно одно значение.

Следует сохранить семантику authoritative specification:

```
Breadcrumb = "Home"
```

Если дальнейшие материалы явно покажут другую требуемую семантику, это должно быть отдельным изменением спецификации, а не предположением scraper.
...
После `.get_text(strip=True)` результат:
...
Из него удалить HTML.

Особенно важно:

- `<br>` превращать в пробел/перенос строки;
- не сохранять `<p>`, `<h4>`, `<strong>`;
- удалить рекламный блок с телефоном, если он не является частью product short description.
В предоставленном HTML рекламный блок находится внутри `.short-description`:

`Advies nodig... Bel onze specialisten...`

**ПРЕДПОЛОЖЕНИЕ:** клиент ожидает именно товарное short description из `DS-PRK-Scraper.json`, поэтому сервисный call-to-action следует исключить. Это необходимо подтвердить при тесте двух товаров.

## imageurl

Источник должен быть **product page**, а не thumbnail category page.

Необходимо собрать все доступные изображения товара.

Порядок:

- найти gallery/image container;
- получить оригинальный `src`, `data-src`, `href` или аналогичный атрибут;
- отбросить thumbnails/cache-варианты, если рядом существует original URL;
- привести URL к абсолютному;
- удалить дубликаты;
...
Это прямо соответствует инструкции:

`if multipli seperated by comma`.

## image_name

Для каждого URL получить filename без query string.
...
```
polar_ge579.jpg,polar_ge579_2.jpg
```

Но конкретное правило именования **не задано окончательно**.

Поэтому:

**ПРЕДПОЛОЖЕНИЕ:** использовать basename оригинального image URL, а при совпадениях добавлять последовательный suffix.

Скачивание изображений является отдельным сетевым действием и должно происходить в `scraper.py`, а не в parser.

Однако существующий контракт `scrape_data(engine) → List[str]` не предусматривает возврат информации о скачанных локальных файлах.

Поэтому до реализации полного image pipeline необходимо проверить существующий `main.py`/exporter: либо изображения должны сохраняться по известному `OUTPUT_DIR`, либо существующий framework должен уже иметь предусмотренный механизм хранения этих файлов.

**Не следует менять контракт **`scrape_data()`**.**
...
Результат должен пройти через существующий `app.utils.clean_price()`.

Не создавать собственный duplicate price cleaner.
...
Это соответствует явному ответу клиента.

## Description

Источник — основной description container product page.

Точный селектор **не представлен в переданном сокращённом **`product-page.html`.

Поэтому нельзя утверждать конкретный CSS selector как уже подтверждённый.

**ПРЕДПОЛОЖЕНИЕ:** основной description находится в стандартном product-info/content block страницы товара.

Алгоритм:

- найти основной description container;
- удалить HTML;
- сохранить текстовые блоки;
- сохранить логические переносы строк;
- не включать HTML markup;
- не смешивать description с unrelated sidebar/recommendation content.
Согласно `DS-PRK-Scraper.json`, description должен быть чистым текстом.

# Specs

В спецификации значение:

```
https://www.awesomescreenshot.com/image/62286448?key=8b032d4304d185282fd860cfb3c858bb each own colom
```

Это не является значением характеристики товара. Это инструкция:

**каждая характеристика должна быть отдельной колонкой.**

Однако authoritative output содержит только одно поле `Specs`, поэтому нельзя самостоятельно создавать новые top-level columns.

Следовательно:

`Specs` должен содержать представление требуемых характеристик внутри этого существующего поля, в соответствии с фактическим форматом проекта/exporter.

Поскольку точный формат не показан в HTML и не определён однозначно в материалах, это остаётся неизвестной частью спецификации.

# Spec_detail

Авторитетная инструкция:
...
- найти первый `:`;
- левая часть — название характеристики;
- правая часть — значение;
- не использовать последующие `:` для разделения;
- сохранить исходный текст значения;
- trim whitespace;
- отсутствующие значения оставить пустыми.
Например:
...
должны интерпретироваться как четыре пары header/value.

Но окончательное машинное представление `Spec_detail` всё ещё не задано явно.

**ПРЕДПОЛОЖЕНИЕ:** наиболее безопасно хранить пары в существующем поле `Spec_detail`, не создавая новые top-level JSON keys.

# 3.6. Финальная структура результата

Один объект должен иметь **ровно 11 полей**, в следующем порядке:
...
- `sku`;
- `brand`;
- дополнительных image fields;
- дополнительных specification columns
добавлять нельзя, поскольку клиент явно зафиксировал структуру.

# 4. Обработка ошибок

СценарийДействиеTimeout categoryИспользовать существующий timeout из `config.py`; повторить ограниченное число раз; после исчерпания retry записать ошибку и остановить scrapeTimeout product pageПовторить переход; после неудачи записать ошибку и пропустить конкретный товар403Записать в существующий logger; прекратить дальнейший агрессивный обход; не пытаться обходить защиту новым HTTP-клиентомCloudflare challengeЗафиксировать в логах; если challenge не проходит автоматически — остановить scrape для ручного вмешательстваCAPTCHAОстановить scrape; записать ошибку`li.item` отсутствуетЗаписать warning; если страница пустая — завершить пагинациюProduct URL отсутствуетПропустить карточкуProduct page не содержит titleВернуть `Title = ""`; не останавливать весь scrapeЦена отсутствует`Price = ""` либо значение, которое поддерживает существующий exporter/schema; предпочтительно `""` для строковой совместимостиSale price отсутствует`Sale price = ""`Description отсутствует`Description = ""`Specs отсутствуют`Specs = ""`, `Spec_detail = ""`Image отсутствует`imageurl = ""`, `image_name = ""`Image download failedЗаписать warning; не терять весь товарДубликат product URLНе скачивать повторноПустая product pageЗаписать ошибку и пропуститьПустой результат категорииЗавершить paginationПовторная страницаОстановить pagination по visited URLRetry не должен превращаться в бесконечный цикл.
...
Количество retry следует брать из существующей конфигурации, если соответствующая настройка уже есть в `config.py`; новую глобальную настройку ради этого модуля создавать не нужно.
...
- Rate Limiting
- CAPTCHA как потенциальный результат Cloudflare
- Infinite Scroll
- Shadow DOM
- iframe как необходимый источник данных
- WebSocket
- GraphQL API
- Сложная/изменяемая DOM-структура
- Lazy loading изображений — возможно, требует проверки
- Изменение структуры характеристик
- Изменение количества страниц/товаров
### Главный риск

Наибольшая техническая неопределённость находится не в pagination, а в:

- полном DOM блока изображений;
- полном DOM блока характеристик;
- точном формате `Specs`;
- точном формате `Spec_detail`;
- способе сохранения скачанных изображений при неизменяемом контракте `scrape_data()`.
# 6. Порядок реализации

## 1. parser.py

Сначала реализовать parser.

Причина: HTML примеры уже предоставлены, поэтому extraction можно тестировать независимо от сети.
...
Затем на сохранённом product HTML:
...
Особое внимание уделить проверке exact output keys.

## 2. parse_html_data()

После `parse_product()` реализовать обработку списка:
...
Не выполнять network operations.

## 3. scraper.py

После готового parser спроектировать сетевой обход:
...
Scraper не должен импортировать parser.

## 4. Интеграционная проверка

Проверить существующий `main.py` без его изменения:
...
Использовать ровно 2 товара.

По актуальному листингу первыми двумя товарами сейчас являются:

- `Polar DM071`
- `Cavanova 605002`
При этом `Polar GE579` также находится на первой странице.

Не следует жёстко кодировать эти названия: тест должен брать первые два уникальных product URL из фактического HTML.
...
- ровно 2 объекта;
- одинаковый набор keys;
- точное написание keys;
- точный порядок keys;
- отсутствие дополнительных полей;
- отсутствие смешивания нескольких характеристик в неожиданных columns;
- `Sale price == ""`, если скидки нет;
- HTML отсутствует в текстовых полях;
- отсутствующие характеристики не ломают запись;
- товары без наличия не отбрасываются;
- image URLs соответствуют скачанным изображениям.
# Что останется неизвестным

После анализа предоставленного prompt и HTML остаются следующие конкретные неизвестные.

### 1. Точный формат Specs

Инструкция говорит, что характеристики должны быть отдельными колонками, но authoritative JSON одновременно содержит только поле `Specs`.

Нельзя самостоятельно придумать новый формат.

### 2. Точный формат Spec_detail

Определено правило:

всё до первого `:` — header.

Но не определено, должен ли результат быть:

- строкой;
- списком;
- объектом;
- каким-либо другим форматом.
Это нужно определить по существующему exporter/schema либо по ожидаемому тестовому output.

### 3. Полный HTML блока характеристик

Предоставленный `product-page.html` обрывается после price box. Сам блок specs отсутствует.

Поэтому конкретные selectors для него нельзя честно назвать подтверждёнными.

### 4. Полный HTML image gallery

В category HTML есть только thumbnail:

```
img[src]
```

Но для требований клиента нужен механизм получения оригинальных product images.

### 5. Правило image_name

Требуется скачивание изображений, но окончательное naming convention не задано.

### 6. Связь локального image файла с imageurl

Контракт scraper фиксирован как:

```
scrape_data(engine) → List[str]
```

Поэтому способ передачи результата скачивания изображений в parser/exporter должен соответствовать уже существующему `main.py` и `config.py`; менять этот контракт нельзя.

### 7. Canonical URL

В предоставленном сокращённом product HTML canonical element отсутствует.

Если `URL` должен гарантированно извлекаться из HTML, необходимо подтвердить наличие canonical либо использовать другой уже существующий элемент страницы.

### 8. Short description cleanup

В `.short-description` одновременно находится товарное описание и рекламный CTA с телефоном.

Нужно проверить на тестовых товарах, должен ли CTA исключаться.
...
API, GraphQL, requests/httpx, Selenium и Scrapy не нужны.
...
- `parse_product(html)`
- внутренние функции очистки текста, цен, изображений и характеристик.
...
Названия и порядок не изменять.

### Главные риски

- Cloudflare.
- HTML changes.
- Неизвестный полный DOM specs.
- Неопределённый формат `Specs`/`Spec_detail`.
- Неопределённая схема хранения скачанных изображений.
- Возможный lazy loading gallery.
**Итоговое архитектурное решение:** не менять существующее ядро; реализовать только `scraper.py` и `parser.py`, причём scraper получает product URLs из listing, сам открывает product pages и возвращает `List[str]`, а parser независимо преобразует эти HTML в строго заданную структуру `DS-PRK-Scraper.json`.
