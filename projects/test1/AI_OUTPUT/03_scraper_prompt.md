...
# Проектирование реализации scraper.py и parser.py

Источник требований: полный `02_project_prompt.md`, включая `DS-PRK-Scraper.json`, HTML-примеры, ограничения существующего ядра и требования к архитектуре. Исходный prompt

## 1. Полный поток данных

Выбранный подход: **PlaywrightEngine для получения страниц + BeautifulSoup для разбора HTML**.
...
HTML страницы категории
        ↓
scraper.py извлекает URL товаров и URL следующей страницы
        ↓
PlaywrightEngine открывает product pages
        ↓
HTML страниц товаров
        ↓
scraper.py при необходимости скачивает изображения
        ↓
...
Для текущего теста scraper должен остановиться после получения **2 товаров**, а не обходить все 429 товаров.

При этом архитектурный контракт сохраняется:

```
scrape_data(engine) → List[str]
```

Каждый элемент результата — HTML одной страницы товара.

Сетевой код не должен передавать в parser никаких объектов Playwright, `Page`, response или browser context.
...
В предоставленных материалах прямо указано, что API отсутствует; `network.har` пуст, а `notes.txt` содержит указание `API нет, и сайт на JS`. Поэтому проектировать API-клиент на основании предположений нельзя.

## Почему HTML + Playwright

Это основной вариант.

`main.py` уже передаёт в `scrape_data()` готовый `PlaywrightEngine`, а сам framework фиксирует Playwright как механизм получения страниц. Следовательно, использовать `requests`/`httpx` или добавлять второй browser stack не требуется и архитектурно неправильно.

Плюсы:

- поддерживается JavaScript;
- используется уже существующий browser context;
- cookies, proxy и user-agent уже находятся в инфраструктуре;
- product pages доступны через обычный HTML;
- не требуется новый сетевой слой;
- хорошо соответствует существующему контракту `scrape_data(engine) → List[str]`.
Минусы:

- браузер тяжелее прямого HTTP-запроса;
- потенциально возникает Cloudflare;
- необходимо учитывать timeout и возможные challenge;
- нужно отдельно контролировать загрузку изображений.
Для данного проекта эти минусы приемлемы.
...
В scraper не следует импортировать `parser.py`.

### _fetch_listing_html(engine, url) → str

**Назначение:** открыть URL категории и вернуть итоговый HTML.

Алгоритм:

- Перейти на URL.
- Дождаться загрузки страницы.
- Дождаться наличия контейнера товаров.
- Получить rendered HTML.
- Вернуть строку HTML.
Основной ориентир ожидания:

```
ul.products-grid.category-products-grid
```

Дополнительный допустимый признак успешной загрузки — наличие:

```
li.item
```

В предоставленном HTML именно `li.item` содержит карточку товара.

### _fetch_product_html(engine, url) → str

**Назначение:** открыть страницу конкретного товара и вернуть HTML.

Перед получением HTML дождаться product page.
...
Наличие `h1` с названием товара является хорошим минимальным признаком того, что product page действительно загрузилась. Структура подтверждена предоставленным `product-page.html`.

### _extract_product_urls(html) → List[str]

**Назначение:** получить URL товаров из HTML listing.

Это допустимая часть scraper, поскольку речь идёт о **навигации**, а не об извлечении бизнес-полей товара.
...
URL из обоих источников следует нормализовать и дедуплицировать.

Предпочтительно брать URL из `h2.product-name a`, если он присутствует, либо использовать `a.product-image`.

На предоставленном HTML оба элемента содержат URL одного товара `polar-dm071.html`.

### _get_next_page_url(html, current_url) → str | None

**Назначение:** определить следующую страницу категории.

На сайте используется обычная пагинация:
...
Следовательно, при полном scraping предпочтительно извлекать фактический `href` ссылки `.next`, а не вручную конструировать номера страниц.

Ориентир:

```ориентир:
div.pager a.next[href]
```

Если `.next` отсутствует — вернуть `None`.

В тестовом режиме эта функция может быть предусмотрена архитектурно, но фактически после набора двух товаров дальнейшая пагинация не нужна.

### _download_product_images(...)

Вспомогательная функция нужна именно для сетевой части, поскольку клиент требует **фактически скачать изображения**, а не только сохранить URL.

**Назначение:**

- получить image URLs из product HTML;
- скачать каждый файл через доступный browser/network context;
- сохранить файл в `OUTPUT_DIR`/предусмотренную конфигурацией директорию;
- использовать детерминированное имя файла;
- не добавлять никаких новых полей в итоговую JSON-схему.
Однако здесь есть архитектурный нюанс: `parser.py` отвечает за извлечение `imageurl` и `image_name`, а scraper — за фактическое скачивание.

Поэтому:

```поэтому:
parser → определяет image URL и имя файла
scraper → физически скачивает файл
```

Чтобы не нарушать ответственность модулей, scraper должен использовать HTML/DOM только для получения необходимых URL изображений и выполнять download как сетевую операцию.

## 3.2. Алгоритм обхода

### Тестовый запуск

- `scrape_data(engine)` получает URL категории из `config.py`/предусмотренной конфигурации.
- Открывает category page.
- Получает HTML.
- Извлекает product URLs.
- Берёт первые два уникальных URL.
- Для каждого URL:

- делает `random_delay()`;
- открывает product page;
- ждёт основной product selector;
- получает HTML;
- скачивает изображения;
- добавляет HTML в результирующий список.
- Возвращает два HTML-документа.
Поскольку клиент явно запросил сначала только два товара, scraper не должен продолжать обход до 429 товаров. Требование тестового этапа подтверждено в prompt.

### Полный запуск после утверждения теста

После подтверждения теста:
...
Пагинация заканчивается, когда:

- отсутствует `a.next`;
- либо следующая страница не содержит `li.item`;
- либо URL следующей страницы уже был обработан.
Это предотвращает бесконечный цикл.

## 3.3. Lazy loading / scroll / tabs

По имеющимся материалам infinite scroll не используется. Пагинация обычная, через `?p=N`.

Поэтому:

- бесконечный scroll не нужен;
- кнопка «Показать ещё» не нужна;
- табы специально открывать не следует, пока фактический DOM не покажет такую необходимость.
### Изображения

Для изображений остаётся проверяемый момент: предоставленный compressed HTML показывает обычный `<img src="...">`, но нельзя заранее утверждать, что абсолютно все изображения на реальной product page представлены одинаково.

Поэтому на реализации необходимо проверить:

```
img[src]
```

и потенциальные варианты:
...
Если реальная страница использует только `src`, дополнительных действий не требуется.

Это именно **ПРЕДПОЛОЖЕНИЕ**, которое должно быть проверено на первых двух товарах.

## 3.4. random_delay()

Использовать уже существующую функцию из `app.utils`, не создавать собственную.

Рекомендуемые точки:

- перед первым переходом на product page;
- между product pages;
- между переходами на страницы категории;
- перед повторной попыткой после timeout.
Не следует делать искусственную задержку после каждого внутреннего DOM-операционного действия.

# 4. Проектирование app/parser.py

Parser полностью offline относительно уже полученного HTML.

Он **не делает сетевых запросов**, не скачивает изображения и не использует Playwright.

## 4.1. Интерфейс функций

ФункцияНазначениеВходВозвращаемое значение`parse_html_data(html_pages)`Обработать список product HTML и сформировать итоговые записи`List[str]``List[dict]``parse_product(html)`Извлечь все поля одного товара`str``dict``_parse_breadcrumb(soup)`Извлечь breadcrumb`BeautifulSoup``str``_parse_title(soup)`Извлечь название`BeautifulSoup``str``_parse_short_description(soup)`Извлечь short description`BeautifulSoup``str``_parse_prices(soup)`Извлечь обычную и sale price`BeautifulSoup``tuple``_parse_description(soup)`Извлечь полное описание как чистый текст`BeautifulSoup``str``_parse_specs(soup)`Извлечь характеристики`BeautifulSoup``str``_parse_spec_detail(soup)`Извлечь заголовки характеристик согласно спецификации`BeautifulSoup``str``_parse_images(soup)`Извлечь image URLs и имена файлов`BeautifulSoup``tuple``_clean_text(element)`Удалить HTML и нормализовать whitespaceHTML element`str``parse_listing(html)` можно сохранить как совместимую функцию для существующего интерфейса `main.py`, но для текущего сценария основной источник полного результата — product pages.

# 5. Спецификация полей

Итоговый словарь обязан иметь **ровно эти поля и в этом порядке**:
...
- переименовывать `Title ` в `Title`;
- удалять пробелы в названиях ключей;
- добавлять `currency`;
- добавлять `SKU`;
- добавлять `availability`;
- объединять `Specs` и `Spec_detail`;
- создавать дополнительные image-поля.
Это прямо следует из authoritative `DS-PRK-Scraper.json`.
...
В предоставленном product HTML цепочка содержит:
...
Но в исходной спецификации пример значения `Breadcrumb` — `Home`.

Поэтому здесь особенно важно **не придумывать формат**. В соответствии с существующим `DS-PRK-Scraper.json` поле должно сохраняться в формате, определённом фактической спецификацией.

**ПРЕДПОЛОЖЕНИЕ:** для текущего результата `Breadcrumb` должен соответствовать уже заданному правилу проекта, а не автоматически превращаться в произвольную строку из всех breadcrumb-узлов. Если отдельное правило не задано, это следует проверить на ожидаемом тестовом output.
...
Особенно важно не включать в short description рекламный блок:

```
Advies nodig, of meerdere stuks tegen de scherpste prijs?
```

если он является отдельным содержательным блоком страницы, а не частью product short description.

**ПРЕДПОЛОЖЕНИЕ:** основным short description является текст первого содержательного `<p>` внутри `.short-description`, поскольку именно он соответствует краткому описанию товара в предоставленном HTML.

При реализации это необходимо подтвердить на реальной product page.

## imageurl

Источник:

изображения product page.
...
HTML сохранять не нужно.

## image_name

Имя каждого скачанного изображения.
...
Имена должны соответствовать реально сохранённым файлам.
...
Значение очищается существующей функцией:

```
app.utils.clean_price()
```

Не дублировать `clean_price()` в parser.
...
Нельзя копировать обычную цену в `Sale price`.

Это прямо задано требованиями клиента.

## Description

Источник — полное описание товара на product page.

Необходимо определить фактический контейнер полного описания на реальном DOM; предоставленный compressed `product-page.html` показывает только верхнюю часть страницы и не содержит полного блока `Productbeschrijving`.

Поэтому конкретный selector полного description **нельзя достоверно назвать на основании предоставленного фрагмента**.

**ПРЕДПОЛОЖЕНИЕ:** вероятным контейнером является блок, соответствующий секции `Productbeschrijving`, однако это должно быть подтверждено реальным HTML.
...
- сохранить смысловые переносы строк между абзацами/списками;
- не добавлять HTML.
# 6. Specs и Spec_detail

Эти два поля требуют особого внимания.

Спецификация говорит:

```
Specs — характеристики; каждая характеристика отдельно.
Spec_detail — текст до ":" используется как заголовок характеристики.
```

Следовательно, parser должен рассматривать характеристики как пары:
...
Внутреннее представление должно сохранять каждую характеристику отдельно, не смешивая значения разных характеристик.
...
```то есть:
Spec_detail = текст слева от первого ":"
```

Не следует разделять значение по последующим двоеточиям, если они встречаются внутри значения.

### Отсутствующие характеристики

Если конкретная характеристика отсутствует:
...
- брать характеристику из другого товара.
Требование оставлять отсутствующие характеристики пустыми явно задано клиентом.

# 7. Финальная структура одного результата

Принципиально структура должна выглядеть следующим образом:
...
Это **не готовый Python-код**, а контракт результата.

Фактические значения должны заполняться только из product HTML.

# 8. Обработка ошибок

СценарийДействиеTimeout при category pageИспользовать существующий timeout из `config.py`, повторить ограниченное число раз; после неудачи записать ошибку через существующий logger и завершить сборTimeout при product pageПовторить переход; после исчерпания retry записать ошибку и пропустить товарHTTP 403Записать в logger; прекратить дальнейшее агрессивное обращение к сайтуCloudflare challengeЗафиксировать в logger и остановить scraping, если challenge не исчезает автоматическиCAPTCHAНе пытаться обходить; записать ошибку и остановить запуск для ручного вмешательстваProduct selector отсутствуетЗаписать warning; проверить, действительно ли загрузилась product page; товар можно пропуститьЦена отсутствуетЗаписать пустое значение, не прекращая обработку товараSale price отсутствует`""`Description отсутствует`""`Отдельная характеристика отсутствует`""`Изображение не скачалосьЗаписать ошибку; не подставлять фиктивный URLНет товаров на listing pageЗаписать warning и завершить пагинациюДублирующийся product URLНе обрабатывать повторноНекорректная следующая страницаЗаписать warning и завершить пагинациюДля API-ошибок/битого JSON специальная обработка не нужна, поскольку API в выбранной архитектуре отсутствует.

# 9. Валидация результата

Перед передачей результата exporter должен получить записи, удовлетворяющие следующим условиям:
...
- URL относится к product page;
- Title взят из товара;
- Description не содержит HTML;
- Sale price пустой при отсутствии скидки;
- отсутствующие характеристики не заполнены выдуманными значениями;
- `imageurl` содержит URL изображений;
- `image_name` содержит соответствующие имена файлов.
### Файлы

Для каждого значения `image_name` должен существовать соответствующий скачанный файл.
...
Несмотря на достаточность данных для проектирования, несколько вещей нельзя достоверно установить из предоставленных материалов.

### 1. Точный selector полного Description

Предоставленный compressed `product-page.html` заканчивается на блоке цены и не содержит полного DOM блока описания.

**ПРЕДПОЛОЖЕНИЕ:** полный текст находится в секции `Productbeschrijving`, как указано в аналитической части prompt. Это нужно подтвердить на реальной странице.

### 2. Точная структура всех Specs

Есть пример характеристик GE579, но нет полного HTML-фрагмента блока `Extra informatie`.

**ПРЕДПОЛОЖЕНИЕ:** характеристики представлены обычными текстовыми строками/табличными элементами с разделителем `:`. Это необходимо проверить на первых двух товарах.

### 3. Все варианты image attributes

Из category HTML подтверждён обычный:

```
img[src]
```

Но нельзя гарантировать, что product gallery не использует `data-src`, thumbnail/full-size URL или lazy loading.

Это следует проверить во время теста.

### 4. Точная политика именования скачанных изображений

Естественный вариант — basename URL, но в предоставленной спецификации нет отдельного правила переименования.

**ПРЕДПОЛОЖЕНИЕ:** использовать имя файла из URL без дополнительного переименования.

### 5. Формат Breadcrumb

Product HTML содержит несколько breadcrumb-элементов, тогда как пример в authoritative JSON содержит `Home`.

Поэтому не следует самостоятельно вводить новый формат breadcrumb без проверки ожидаемого результата.

### 6. Реальное поведение Cloudflare

Наличие `cf_clearance` подтверждено, но это не означает, что каждый запуск потребует challenge. Нельзя заранее утверждать, что дополнительный механизм обхода Cloudflare понадобится.

### 7. Полный набор товаров для теста

Материалы содержат `Polar DM071` и `Polar GE579` как примеры товаров. Для реализации лучше не хардкодить эти названия: scraper должен взять первые два уникальных product URL из фактически загруженного listing.

# 11. Оценка рисков

- **Cloudflare** — основной риск, средний.
- **Login** — не требуется для scraping.
- **Rate limiting** — возможен, точный лимит неизвестен.
- **CAPTCHA** — присутствует на форме отзывов, но не является частью штатного scraping flow.
...
- **Изменение DOM** — средний долгосрочный риск.
- **Lazy loading изображений** — пока не подтверждено.
- **Разные наборы Specs у товаров** — средний риск.
Особое внимание нужно уделить Cloudflare и изображениям. Остальная задача относительно прямолинейна.

# 12. Порядок реализации

Здесь есть важное отличие от шаблонного порядка из prompt.

Поскольку `scraper.py` **не должен импортировать parser.py**, нельзя строить scraper вокруг вызова parser.

Оптимальный порядок:

### 1. parser.py

Сначала реализовать offline-логику:

- `parse_product`;
- extraction базовых полей;
- extraction description;
- extraction specs;
- extraction images;
- очистку текста;
- формирование точной структуры из 11 полей.
Причина: parser можно тестировать на сохранённых HTML без сети.

### 2. scraper.py

Затем реализовать:

- открытие category page;
- extraction product URLs;
- открытие product pages;
- тестовый лимит 2 товара;
- получение HTML;
- скачивание изображений;
- pagination для будущего полного запуска.
### 3. Интеграционная проверка с существующим main.py

`main.py` менять не нужно.

Проверяется только соответствие существующему контракту:
...
Запустить реальный scraper на двух товарах и проверить:

```
2 product pages
2 JSON records
изображения физически скачаны
11 полей в каждом record
```

### 6. После approval

Снять тестовый лимит `2` и включить полный pagination flow на все товары категории.
...
- вспомогательная логика скачивания изображений.
`parser.py`**:**

- `parse_html_data(html_pages)`
- `parse_product(html)`
- функции извлечения breadcrumb/title/description/prices/specs/images;
- очистка HTML и текста.
**Поток:**
...
**Итоговая структура:** строго 11 существующих полей:
...
**Главные риски:** Cloudflare, фактический DOM блока Description/Specs, lazy loading изображений и возможные различия шаблонов product pages.

**Критически важно:** `main.py`, `browser.py`, `config.py`, `utils.py` и `exporter.py` менять не требуется. Готовый Python-код также не требуется и по условиям задания не должен генерироваться.
