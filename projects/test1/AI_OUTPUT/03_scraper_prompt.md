...
# Анализ проекта Scraper — Professionele-Koeling.nl

## 1. Краткое описание задачи

Клиент хочет получить scraper для Professionele-Koeling.nl, который собирает данные о товарах из категории Koelkasten&Kisten.

На текущем этапе требуется собрать 2 товара и предоставить тестовый JSON. Структура JSON должна строго соответствовать DS-PRK-Scraper.json.

Необходимо собирать текстовые данные, цены, характеристики и изображения. Изображения требуется скачивать. Отсутствующие значения должны оставаться пустыми, а товары без наличия также должны включаться.

## 2. Конечный результат

JSON со строго заданными полями:
...
- Specs
- Spec_detail
Порядок и названия полей изменять нельзя.

## 3. Рекомендуемое решение

Использовать существующий PlaywrightEngine проекта совместно с HTML parsing.
...
- очистить описание от HTML;
- разобрать характеристики;
- найти и скачать изображения;
- сформировать JSON;
- проверить результат на двух товарах.
## 4. Почему другие варианты хуже

Requests/BeautifulSoup технически могут быть достаточны для части сайта, но существующий framework уже использует PlaywrightEngine.

Scrapy и Selenium избыточны. Отдельный API-слой не оправдан, поскольку API в предоставленных материалах не обнаружен.

## 5. Анализ сайта

Pagination: есть.

JavaScript: присутствует, но данные доступны в HTML.

API/GraphQL: не обнаружены.

Cookies: присутствуют.

Cloudflare: есть признаки, включая cf_clearance.

Infinite Scroll: не обнаружен.

Login: не требуется по предоставленным данным.

JWT/Bearer Token: не обнаружены.

XHR/Fetch, WebSocket, Lazy Loading, Sitemap и Rate Limits: достоверно не определены из предоставленных материалов.

## 6. Что необходимо получить

Перед реализацией желательно уточнить:

- точный формат Specs/Spec_detail;
- правила хранения и именования скачанных изображений;
- какие URL изображений использовать в imageurl.
Исходную Google Sheet получать не нужно: DS-PRK-Scraper.json является авторитетной спецификацией.
...
- изменение HTML;
- истечение cookies;
- различие между thumbnail и оригинальными изображениями;
- неоднозначность структуры характеристик;
- изменение количества товаров и страниц.
Старый checkpoint нельзя использовать как источник актуального количества товаров.

## 8. Вопросы клиенту

- Какой точный формат должен иметь Specs/Spec_detail в JSON?
- Куда и с каким правилом именования сохранять скачанные изображения?
- В imageurl использовать URL оригинального изображения или URL thumbnail?
## 9. Стек
...
Цель: проверить существующий PlaywrightEngine и pipeline.

Результат: понятен контракт scraper.

Зависимость: существующий код проекта.

### Этап 2

Цель: получить товары и pagination.

Результат: корректные product URLs.

Зависимость: category HTML.

### Этап 3

Цель: извлечь все поля товара.

Результат: нормализованная запись.

Зависимость: уточнение Specs/Spec_detail.

### Этап 4

Цель: скачать изображения.

Результат: локальные файлы и соответствующие значения imageurl/image_name.

Зависимость: правило хранения изображений.

### Этап 5

Цель: собрать 2 тестовых товара.

Результат: тестовый JSON.

Зависимость: предыдущие этапы.

### Этап 6

Цель: проверить структуру и содержимое JSON.

Результат: готовый тестовый файл.

### Этап 7

Цель: после подтверждения клиента выполнить полный scrape.

Результат: полный JSON.

Зависимость: approval тестового результата.

## 11. Оценка сложности

Сложность: 4/10.

Тестовая реализация: примерно 3–5 часов.

Полная реализация: примерно 6–10 часов.

Вероятность блокировок: средняя.

Вероятность необходимости браузера: высокая, поскольку PlaywrightEngine зафиксирован framework.

Вероятность будущих изменений сайта: средняя.
...
Не требуется API, GraphQL, Scrapy, Selenium, база данных или сложная multi-layer архитектура.

Оптимальная схема:

PlaywrightEngine → HTML → parsing → images → JSON.

Также следует проверить, можно ли получить все необходимые поля из category page. Если нет, для текущей спецификации нужно переходить на product pages.

## 13. Итоговая рекомендация

Использовать существующий PlaywrightEngine и простой HTML parsing.

Стратегия оптимальна, поскольку соответствует framework, не требует API и не создаёт ненужной инфраструктуры.

Перед кодированием нужно закрыть три вопроса: формат Specs/Spec_detail, правила сохранения изображений и выбор URL изображений.

После получения этих ответов можно переходить к реализации. До этого писать код не следует.

Уверенность в общей стратегии: высокая. Уверенность в деталях Specs/Spec_detail и image handling: средняя.
...
Прочитал prompt целиком и сверил требования, входные материалы и ограничения архитектуры. Ниже — проектирование реализации **без готового Python-кода**, строго для `app/scraper.py` и `app/parser.py`, без изменения ядра. GitHub+1


## 1. Почему выбран именно этот способ


### Выбор: Playwright + HTML + BeautifulSoup


API использовать не следует: в предоставленных материалах `network.har` пуст, а `notes.txt` прямо указывает, что API отсутствует. Кроме того, проект уже принудительно использует `PlaywrightEngine`, поэтому `requests`/`httpx` для `scraper.py` не подходят по архитектурному контракту. GitHub


Схема:


**PlaywrightEngine → HTML → BeautifulSoup → dict → list[dict] → существующий exporter**


Playwright нужен для навигации и получения уже отрендеренного HTML. BeautifulSoup используется только для разбора полученного HTML.
...
- не требует нового HTTP-клиента;

- подходит для JS-сайта;

- позволяет обходить product pages;

- не требует Scrapy, БД или отдельного crawler;

- удобно тестируется на сохранённом HTML.
...
- браузер тяжелее прямого HTTP;

- остаётся риск Cloudflare;

- DOM-селекторы могут измениться;

- необходимо отдельно скачивать изображения.


На основании имеющихся данных это наиболее простой и совместимый вариант. GitHub+1
...
Для текущего теста нужен только **2 товара**, поэтому нет необходимости обходить все 429 товара.


URL категорииhttps://www.professionele-koeling.nl/koelkasten-kisten.html        ↓PlaywrightEngine        ↓HTML категории        ↓scraper._extract_product_urls()        ↓2 product URL        ↓PlaywrightEngine        ↓HTML product page #1HTML product page #2        ↓scrape_data(engine) → List[str]        ↓main.py        ↓parse_html_data(raw_pages_content)        ↓BeautifulSoup        ↓parse_product(html)        ↓dict для каждого товара        ↓List[dict]        ↓exporter.py        ↓JSON
Для scraper правильнее вернуть **только product pages**, а не смешивать category HTML и product HTML. Это позволяет `parse_html_data()` однозначно обработать каждый элемент как страницу товара и не требует изменения `main.py`.


Ключевой контракт остаётся неизменным:


`scrape_data(engine) → List[str]`


`parser.py` при этом вообще не импортируется в `scraper.py`. GitHub
...
Главная функция, которую вызывает существующий `main.py`.


**Назначение:** собрать HTML двух product pages.
...
- Получить URL категории из конфигурации/имеющегося входа.

- Открыть категорию через предоставленный `PlaywrightEngine`.

- Дождаться появления товарных карточек.

- Получить HTML страницы.

- Из HTML извлечь URL первых двух товаров.

- Для каждого URL открыть product page.

- Дождаться основных элементов product page.

- Получить полный HTML.

- Вернуть список из двух HTML-документов.


Для тестового этапа:


scrape_data(engine)    ↓listing HTML    ↓_extract_product_urls()    ↓[product_url_1, product_url_2]    ↓_fetch_product_html()    ↓[product_html_1, product_html_2]

### _fetch_listing_html(engine, url) → str


**Назначение:** открыть категорию и получить её HTML.
...
Перед возвратом необходимо дождаться товарной сетки.


Из предоставленного HTML подтверждён контейнер:


`ul.products-grid.category-products-grid.itemgrid.itemgrid-adaptive.itemgrid-3col.centered.hover-effect.equal-height`


А карточка товара находится в:
...
**Назначение:** навигационное извлечение ссылок на товары.


Это допустимое исключение из общего правила "scraper не занимается бизнес-парсингом": URL нужны scraper исключительно для дальнейшей навигации.
...
или, как более надёжный fallback:


`li.item → h2.product-name → a[href]`


В предоставленном HTML обе ссылки ведут на один и тот же product URL. GitHub
...
- нормализовать относительные URL, если они встретятся;

- удалить дубликаты;

- вернуть первые два URL.


Поскольку текущая задача — тест двух товаров, после получения двух уникальных URL дальнейший обход категории не нужен.
...
**Назначение:** получить HTML конкретного товара.


Перед получением HTML следует дождаться ключевого элемента страницы товара, например:


`div.product-name h1`


Дополнительно можно дождаться:
...
Это подтверждённые элементы предоставленного product HTML. GitHub


Функция возвращает весь HTML страницы, а не отдельные найденные поля.
...
Для **текущего теста из двух товаров функция фактически не нужна**.


Если scraper позднее расширяется на все товары, она должна использовать обычную HTML-пагинацию:
...
и ссылки на следующие страницы. GitHub


Поэтому для полного scraping предпочтительнее определять следующую страницу по ссылке `.pager .next[href]`, а не самостоятельно вычислять номер страницы.
...
Открыть category URL        ↓Дождаться li.item        ↓Получить HTML        ↓Извлечь product URLs        ↓Взять первые 2 уникальных URL        ↓Product #1 → HTML        ↓Product #2 → HTML        ↓Вернуть [HTML1, HTML2]
### Полный режим после утверждения теста


Страница категории        ↓Извлечь product URLs        ↓Посетить каждый product URL        ↓Собрать HTML        ↓Проверить ссылку .pager .next        ↓Если next существует → следующая страница        ↓Повторять до отсутствия next
На основании входных данных infinite scroll не требуется: имеется обычная пагинация с параметром `p`. GitHub+1


### Lazy loading


Для самих product pages нельзя пока утверждать, что все изображения представлены обычным `src`. Это специально обозначено как то, что необходимо проверить при разработке.


Поэтому scraper должен:



- дождаться загрузки product page;

- при необходимости выполнить небольшой scroll страницы;

- только затем получить HTML.


Однако сложный механизм infinite-scroll или ручная эмуляция lazy loading пока не требуется.
...
- после загрузки category page перед переходом к product pages;

- между двумя product pages;

- при дальнейшем полном scraping — между последовательными страницами.


Не создавать собственный механизм задержек.


`browser.py`, `config.py` и `utils.py` не дублировать. GitHub
...
ФункцияНазначениеВходные параметрыВозвращаемое значение`parse_html_data(html_pages)`Обрабатывает список полученных product pages`List[str]``List[dict]``parse_product(html)`Извлекает все поля одного товара`str``dict``_parse_breadcrumb(soup)`Извлекает Breadcrumb`BeautifulSoup``str``_parse_title(soup)`Извлекает название`BeautifulSoup``str``_parse_short_description(soup)`Извлекает short description`BeautifulSoup``str``_parse_prices(soup)`Извлекает обычную и sale price`BeautifulSoup`два значения`_parse_description(soup)`Извлекает полное описание`BeautifulSoup``str``_parse_specs(soup)`Извлекает характеристики`BeautifulSoup`значение для `Specs`/`Spec_detail``_parse_images(soup)`Находит изображения`BeautifulSoup`URL + имена файлов`_clean_text(element)`Удаляет HTML и нормализует текстHTML element`str`
Ни одна parser-функция не делает сетевых запросов.
...
Финальная структура должна **строго сохранить имена и порядок**, включая пробелы в `Title ` и `Description `. Добавлять `sku`, `currency`, `availability` и другие поля нельзя. GitHub
...
Здесь необходимо соблюдать спецификацию DS-PRK и не создавать дополнительные поля для отдельных breadcrumb levels.


Если ожидаемый формат — полный breadcrumb, его следует сохранить как единое значение.
...
`<br>` преобразуется в пробел/перенос строки, после чего выполняется нормализация whitespace.


Важно не смешивать promotional block:


`Advies nodig, of meerdere stuks...`


с собственно short description, если он структурно находится отдельно от основного текста. В предоставленном HTML он находится внутри того же контейнера, поэтому это место необходимо проверить на реальном DOM и не добавлять предположение как факт. GitHub
...
Извлечь все изображения товара.


URL каждого изображения сохраняется строкой.


Если изображений несколько — разделять запятой, как требует спецификация.
...
Для каждого скачиваемого изображения определить имя файла из URL/path.


Если изображений несколько — имена разделять запятой.


Важно: это не только извлечение URL. Требование клиента — **физически скачать изображения**. Но сами HTTP-запросы за изображениями должны оставаться ответственностью scraper/browser layer, а parser только определяет URL и ожидаемое имя файла.
...
На странице категории аналогичная структура:
...
Используется существующая `utils.clean_price()`.


Не реализовывать собственный очиститель цены.
...
Если `special-price` отсутствует — значение **пустое**, а не копия обычной цены.
...
Источник должен соответствовать основному блоку полного описания товара, в частности `Productbeschrijving`, если именно этот контейнер подтверждается полным HTML.
...
- без `<script>`, `<style>` и служебной разметки;

- с сохранением логических абзацев;

- с нормализацией лишних пробелов.


HTML нельзя сохранять, поскольку это прямо запрещено спецификацией. GitHub
...
Извлекаются характеристики из блока `Extra informatie`.


Каждая характеристика должна обрабатываться отдельно и не смешиваться с соседней.


Пример исходных данных:


Artikelnummer: GE579Inhoud: 29 literTemperatuurbereik: 3-5°C ...Afmetingen BxDxH: 400 x 430 x 530 mmEnergieklasse: F

### Spec_detail


Для каждой характеристики используется текст **до первого :** как название характеристики.
...
При отсутствии характеристики соответствующее значение остаётся пустым.


**Важное замечание:** сама спецификация содержит только поля `Specs` и `Spec_detail`, но одновременно говорит, что каждая характеристика должна быть отдельной. Она не даёт однозначного формального определения, должны ли эти два поля быть строками, массивами или повторяемыми экспортными значениями. Поэтому тип внутреннего представления `Specs`/`Spec_detail` является единственным оставшимся архитектурным вопросом и его нельзя самовольно переинтерпретировать. GitHub
...
На уровне логической модели каждый товар должен содержать **только** следующие поля и в таком порядке:


{    URL    Breadcrumb    Title     Short description    imageurl    image_name    Price    Sale price    Description     Specs    Spec_detail}
Для теста итоговый массив содержит ровно два таких объекта.
...
- любые технические metadata.


Это особенно важно, поскольку исходный prompt прямо запрещает добавлять или переименовывать колонки. GitHub



# 7. Скачивание изображений


Здесь есть важное архитектурное разделение.


`scraper.py` отвечает за сеть, поэтому именно он должен выполнять фактическую загрузку файлов.


Логика:


Product HTML   ↓найдены image URLs   ↓определены имена файлов   ↓скачать изображения   ↓сохранить в OUTPUT_DIR   ↓parser формирует imageurl/image_name
При этом конкретный способ скачивания нужно согласовать с уже существующими возможностями `PlaywrightEngine`/ядра, а не создавать второй HTTP stack.
...
- доступен ли оригинальный image URL без авторизации;

- не является ли `src` thumbnail;

- есть ли `data-src`/другой lazy-loading атрибут;

- как получить оригинальное изображение вместо `295x295` thumbnail.


В предоставленном category HTML точно виден thumbnail URL `295x295`, поэтому использовать его автоматически как финальное изображение нельзя без проверки product page. GitHub
...
СценарийДействиеTimeout category pageПовторить ограниченное число раз; после исчерпания retry записать ошибку и остановить тестTimeout product pageПовторить; если конкретный товар не загрузился — залогировать и пропустить егоHTTP 403 / Cloudflare challengeЗалогировать; не пытаться автоматически обходить challenge; остановить scrapingCAPTCHAЗалогировать и остановить scraping для ручного вмешательстваНет `li.item`Считать страницу пустой/изменённой, залогироватьНет product URLНе создавать фиктивный URL; залогировать карточкуНет titleОставить поле пустым и продолжитьНет priceОставить соответствующее поле пустымНет sale price`Sale price = ""`Нет description`Description = ""`Нет specsсоответствующие значения пустыеНет изображения`imageurl`/`image_name` пустыеОшибка скачивания изображенияЗалогировать, не ломать обработку остальных полейПустая product pageЗалогировать и пропустить товарДубликат product URLНе посещать повторноНеожиданный DOMНе падать на всём scraping; записать ошибку конкретного поля
Особенно важно: отсутствие одного поля не должно уничтожать весь товар.
...
- **Cloudflare** — основной риск, поскольку присутствует `cf_clearance`.

- **Login** — не требуется для scraping по имеющимся данным.

- **Rate Limiting** — точный лимит неизвестен.

- **CAPTCHA** — присутствует на форме отзывов, но не является обязательной частью scraping.

- **Infinite Scroll** — признаков нет.

- **Shadow DOM** — признаков нет.

- **iframe** — критических данных в iframe не обнаружено.
...
- **Lazy-loading изображений** — пока не подтверждено.

- **Разные наборы Specs у товаров** — ожидаемо возможны.


Данные о структуре сайта, пагинации и наличии Cloudflare подтверждены предоставленными материалами. GitHub+1



# 10. Что останется неизвестным


На момент проектирования **блокирующих неизвестных нет**, но следующие вещи необходимо проверить непосредственно во время реализации:



- **Точный DOM блока полного Description.**

- **Точный DOM Extra informatie у разных товаров.**

- **Оригинальные URL изображений**, а не thumbnails.

- Используется ли `src`, `data-src` или другой lazy-loading атрибут.

- Можно ли скачать оригинальные изображения через предоставленный browser context.

- Требуется ли дополнительный scroll для загрузки всех изображений.

- Как Cloudflare поведёт себя при последовательном открытии product pages.

- Точный тип представления `Specs`/`Spec_detail` в финальном JSON при наличии нескольких характеристик.


Последний пункт нельзя решать предположением: prompt запрещает изменять или переосмысливать структуру `DS-PRK-Scraper.json`. GitHub
...
Начать с parser, потому что он полностью изолирован от сети.


Тестировать на сохранённом `product-page.html`.
...
затем специализированные extraction-функции.


Особое внимание — ценам, Description, Specs и очистке HTML.
...
- получить первые два URL;

- открыть две product pages;
...
scrape_data(engine)        ↓List[str]        ↓parse_html_data(...)        ↓List[dict]        ↓exporter.py
### 4. Проверка изображений


Отдельно убедиться, что файлы реально появляются на диске и `image_name` соответствует сохранённым файлам.


### 5. Финальная валидация двух товаров
...
- отсутствие потерянных Specs;

- изображения физически скачаны;

- JSON валиден.


Только после успешной проверки тестового файла имеет смысл расширять scraper до всех 429 товаров. GitHub
...
**Технология:** существующий `PlaywrightEngine` для навигации и получения HTML + BeautifulSoup для parsing.
...
- при полном scraping — `_get_next_page_url()`
...
- отдельные функции для breadcrumb, title, description, prices, images и specs.


**Поток:**


`category → 2 product URLs → 2 product HTML → parser → 2 dict → exporter → JSON`


**Итоговая структура:** строго `URL`, `Breadcrumb`, `Title `, `Short description`, `imageurl`, `image_name`, `Price`, `Sale price`, `Description `, `Specs`, `Spec_detail`.


**Главные риски:** Cloudflare, lazy-loading/оригинальные изображения, изменение DOM и неоднозначность внутреннего представления нескольких `Specs`/`Spec_detail`.


Критически важно: **main.py, browser.py, config.py, utils.py и exporter.py менять не нужно и не следует**. Решение полностью укладывается в зафиксированный контракт существующего ядра. GitHub
