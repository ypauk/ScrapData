...
Для проекта выбирается PlaywrightEngine + HTML/DOM + BeautifulSoup.

Причина: фреймворк уже жестко предоставляет PlaywrightEngine в scrape_data(engine), а контракт требует вернуть List[str] с HTML-страницами. В предоставленных материалах API не обнаружен, HAR пустой, а необходимые данные присутствуют непосредственно в HTML DOM. 
...
HTML category page
        ↓
извлечение URL товарных страниц
        ↓
...
Для текущего теста достаточно получить две товарные страницы. Пагинацию реализовать в архитектуре сразу, но при тестовом запуске не переходить на страницу 2 после получения двух товаров. Категория использует обычную пагинацию через ?p=2, ?p=3 и т. д.; в предоставленном HTML указано 429 товаров по 36 на страницу. 

2. Почему выбран именно HTML + Playwright
Playwright

Playwright обязателен не потому, что доказано наличие сложного SPA, а потому что это зафиксированная технология существующего фреймворка: scrape_data(engine) получает готовый PlaywrightEngine, уже содержащий browser context, cookies, proxy и user-agent. Использование requests/httpx противоречило бы контракту ядра. 

Плюсы:

соответствует существующему main.py;

учитывает browser context и cookies;

подходит для возможного JavaScript;

позволяет корректно работать с Cloudflare/browser state;

не требует нового транспортного слоя.

Минусы:

медленнее обычного HTTP-клиента;

выше расход ресурсов;

возможны timeout/Cloudflare-блокировки;

DOM может измениться.

HTML + BeautifulSoup

HTML выбран источником бизнес-данных, поскольку предоставленный category/product HTML уже содержит ссылки, названия, цены, breadcrumb и short description. 

BeautifulSoup используется исключительно в parser.py для обработки уже полученного HTML. Это соответствует разделению ответственности ядра. 

Плюсы:

простая реализация;

детерминированный парсинг сохраненного HTML;

легко тестировать без сети;

не нужен API/Scrapy/Selenium.

Минусы:

зависимость от DOM-селекторов;

часть структуры Specs/Spec_detail не представлена в переданном product HTML;

изменение Magento-шаблона может потребовать корректировки селекторов.

3. Проектирование app/scraper.py
3.1. Интерфейс функций
scrape_data(engine) → List[str]

Главная функция и единственная функция, вызываемая main.py.

Назначение: собрать HTML категории и нужных товарных страниц.
...
Дождаться основного контейнера товарной выдачи.

Получить HTML категории.

Из HTML извлечь URL товаров.

Взять первые два URL для текущего теста.

Для каждого URL открыть product page.

Дождаться основного product content.

Получить полный HTML страницы товара.

Вернуть List[str] с HTML двух product pages.

Не передавать данные в parser из scraper.

Для будущего полного scrape функция должна продолжать обход страниц через ?p=N, пока не будут обработаны все товары.

_fetch_listing_html(engine, url) → str

Назначение: открыть страницу категории и вернуть ее HTML.
...
Перед извлечением контента необходимо дождаться товарного списка:

ul.products-grid.category-products-grid

В предоставленном HTML карточки находятся внутри:

ul.products-grid.category-products-grid.itemgrid.itemgrid-adaptive.itemgrid-3col.centered.hover-effect.equal-height
    └── li.item

Это подтверждено переданным category HTML. 

_extract_product_urls(html) → List[str]

Назначение: извлечь URL товарных страниц из HTML категории.

Это допустимо в scraper.py, поскольку это навигационная логика, прямо разрешенная контрактом.
...
В предоставленном HTML оба элемента указывают на product URL polar-dm071.html. 

URL следует нормализовать до абсолютного URL.

Для теста:

product_urls[:2]

Не следует выбирать товары по названию, цене или другим бизнес-полям — достаточно порядка выдачи категории.

_fetch_product_html(engine, url) → str

Назначение: открыть страницу товара и вернуть полный HTML.
...
Перед чтением HTML следует дождаться product title:

div.product-name h1

или основного product container.

Предоставленный HTML подтверждает наличие:
...
div.pager div.pages li.next a[href]

В предоставленном HTML next ведет на:

koelkasten-kisten.html?p=2

Следовательно, отдельный клик Playwright не нужен: достаточно определить следующий URL и снова вызвать _fetch_listing_html. 

Для теста: функция фактически не должна использоваться после получения двух товаров.

Для полного scrape: цикл прекращается, когда next отсутствует либо когда новая страница не содержит новых product URLs.

3.2. Паузы

random_delay() из app.utils использовать:

между переходом с category page на product page;
...
Для теста двух товаров достаточно умеренных пауз. Их конкретные значения должны оставаться в зоне существующего utils.py, а не дублироваться в scraper.

3.3. Lazy loading / scroll / clicks

На основании предоставленного HTML:

infinite scroll — не требуется;

обычный scroll — не требуется;

Load more — не обнаружен;

раскрытие карточек — не требуется;

переключение табов — не подтверждено;

пагинация — обычная URL-пагинация.

Поэтому не следует добавлять искусственный scrolling или клики.

4. Проектирование app/parser.py

Parser не выполняет никаких сетевых операций.

Он получает HTML и работает только с ним через BeautifulSoup.

4.1. Интерфейс функций
Функция	Назначение	Вход	Возврат
parse_html_data(raw_pages_content)	Обработать список product HTML и сформировать итоговые записи	List[str]	List[dict]
parse_product(html)	Извлечь один товар из product HTML	str	dict
_parse_breadcrumb(soup)	Извлечь breadcrumb	BeautifulSoup	str
_parse_title(soup)	Извлечь Title	BeautifulSoup	str
_parse_short_description(soup)	Извлечь чистый Short description	BeautifulSoup	str
_parse_prices(soup)	Извлечь Price и Sale price	BeautifulSoup	tuple
_parse_description(soup)	Извлечь чистый Description	BeautifulSoup	str
_parse_specs(soup)	Извлечь Specs	BeautifulSoup	str/пустое значение
_parse_spec_detail(soup)	Извлечь Spec_detail	BeautifulSoup	str/пустое значение
_parse_images(soup)	Извлечь image URL и имена	BeautifulSoup	tuple

При необходимости внутренние helper-функции можно оставить приватными; публичный контракт ядра должен оставаться совместимым с main.py.

5. parse_html_data(raw_pages_content)

Вход:

List[str]

где каждый элемент — HTML отдельной product page.
...
добавить полученный dict в список.

Вернуть List[dict].

Если один product HTML поврежден, ошибка конкретного товара не должна уничтожать уже успешно обработанные товары. Ошибка должна логироваться через существующий logger.

6. parse_product(html)

Для каждого товара формировать ровно следующие поля и именно в указанном порядке:
...
Пробелы в Title и Description являются частью спецификации и должны сохраниться. Нельзя нормализовать их в Title/Description. DS-PRK-Scraper.json прямо объявлен авторитетной спецификацией. 

7. Спецификация каждого поля
URL
...
Важный архитектурный момент: текущий HTML product page в предоставленном фрагменте не показывает отдельный canonical URL.

ПРЕДПОЛОЖЕНИЕ: parser должен брать URL из canonical <link> или другого явно присутствующего URL-элемента product page.

Если такого элемента в реальном HTML нет, поле должно быть получено иначе только после подтверждения фактического DOM. Не следует выдумывать selector.

Для надежной реализации лучше, чтобы scraper передавал только HTML, поэтому URL должен присутствовать в самом HTML либо быть доступен через существующий HTML-контекст.
...
Для предоставленного товара структура:

Home
Koelkasten&Kisten
Polar GE579

Но DS-PRK-Scraper.json показывает ожидаемое значение-пример:

Home

Поэтому нельзя самостоятельно выбирать между полной цепочкой и последним/первым элементом.

ПРЕДПОЛОЖЕНИЕ: для соответствия предоставленной спецификации Breadcrumb должен содержать именно представление, определенное фактическим примером спецификации — Home.

Это единственное поле, для которого требуется проверить ожидаемую семантику перед полным scrape.
...
Ожидаемый результат для предоставленного товара:

Polar GE579

Удаляются только окружающие пробелы и HTML-разметка.
...
Текст необходимо извлекать как plain text.

<br> превращается в пробел/перенос строки в зависимости от принятой общей нормализации текста.

Важно не включать рекламный блок:

Advies nodig, of meerdere stuks tegen de scherpste prijs?
Bel onze specialisten: ...

ПРЕДПОЛОЖЕНИЕ: short description должна соответствовать непосредственно описанию товара, а не всему содержимому контейнера div.short-description. В переданном HTML рекламный блок находится внутри того же контейнера, поэтому его точный DOM-отсев нужно проверить на реальной странице. 

imageurl

Нужно собрать все изображения товара.

Если изображений несколько:

url1,url2,url3

Именно через запятую, согласно DS-PRK-Scraper.json. 

Не смешивать URL с именами файлов.

image_name

Для каждого image URL определить соответствующее имя файла.
...
В предоставленном product HTML:

€ 255,99

После использования существующего clean_price() результат должен быть числовым значением:

255.99

Не реализовывать собственный clean_price() в parser.py.
...
Если .special-price отсутствует, вернуть пустое поле, а не обычную цену. Это явно установлено клиентом. 

Description

Нужен только чистый текст без HTML.

Источник должен быть основным блоком полного описания товара.

Не следует автоматически брать весь <body> или объединять Description с Short description.

Проблема: переданный сокращенный product-page.html заканчивается на блоке цены и не содержит полного DOM Description. Поэтому конкретный selector Description нельзя достоверно определить из имеющихся материалов.

ПРЕДПОЛОЖЕНИЕ: реальная product page содержит отдельный стандартный Magento-блок description, который нужно выбрать после просмотра полного HTML.

Не выдумывать его selector заранее.

Specs

Поле должно содержать характеристики отдельно, а не объединять произвольные данные.

DS-PRK-Scraper.json содержит явное правило:

Specs — характеристики; Spec_detail — пары характеристика/значение, где текст до : является названием характеристики. 

Однако предоставленный сокращенный product HTML не содержит блока характеристик.

Поэтому:

если блок отсутствует → ""

На текущем этапе нельзя надежно назвать конкретный CSS selector.
...
должна логически давать:

header = "Artikelnummer"
value = "GE579"

То есть часть до первого : — название характеристики.

При нескольких характеристиках каждая пара должна оставаться отдельной характеристикой и не превращаться в один произвольный текстовый blob.

Но фактическая структура HTML блока характеристик не предоставлена.

ПРЕДПОЛОЖЕНИЕ: после получения полного product HTML helper будет находить строки характеристик по реальной DOM-структуре, а затем разделять каждую строку по первому :.

До подтверждения DOM selector не фиксировать.

8. Итоговая структура одного результата
{
  "URL": "https://www.professionele-koeling.nl/koelkasten-kisten/polar-ge579.html",
  "Breadcrumb": "Home",
  "Title ": "Polar GE579",
  "Short description": "De Polar GE579 is een zwarte minibar koelkast met 29 liter inhoud voor gebruik in hotelkamers, B&B's of vergaderruimtes. Vrijwel geluidsloos met 2 roosters en 2 deurrekken.",
...
Это структурный пример, а не готовый scrape-результат. URL изображений и остальные отсутствующие в предоставленном HTML значения нельзя выдумывать.

9. Важная проблема с требованием скачивания изображений

Здесь есть архитектурное противоречие, которое необходимо явно зафиксировать.

Клиент требует:

скачать изображения локально;

записать imageurl;

записать image_name.

При этом существующий контракт требует:
...
работает только с уже полученным HTML.

Следовательно, parser не может скачать изображения.

Одновременно scraper не должен знать бизнес-структуру HTML и не должен заниматься extraction изображений.

Поэтому при строго неизменяемых интерфейсах ядра полноценное скачивание изображений невозможно гарантировать только силами scraper.py + parser.py.

Это не повод менять main.py, config.py или exporter — они явно объявлены неизменяемыми.

Рекомендуемое решение в рамках ограничений

Рассматривать скачивание изображений как часть сетевого сбора в scraper.py, но только если существующий PlaywrightEngine уже предоставляет поддерживаемый механизм сохранения загруженного resource/file и существующий OUTPUT_DIR допускает такое сохранение.

Если такой возможности в PlaywrightEngine нет, требование клиента невозможно полностью выполнить без изменения архитектурного контракта, а изменение main.py запрещено prompt.

Поэтому перед реализацией необходимо проверить только существующие возможности PlaywrightEngine, не добавляя новый downloader.

10. Обработка ошибок
Сценарий	Действие
Timeout	Повторить ограниченное число раз; при окончательном отказе записать ошибку через существующий logger и пропустить страницу
HTTP 403	Записать ошибку; прекратить текущий scrape, чтобы не усиливать блокировку
Cloudflare challenge	Записать ошибку и остановить scrape; не пытаться обходить защиту
CAPTCHA	Записать ошибку и остановить scrape, требуется ручное вмешательство
Нет товара	Записать предупреждение; завершить текущую ветку пагинации
Нет цены	Вернуть пустое значение/None, не падать всем parser
Нет Sale price	Sale price = ""
Нет изображения	imageurl = "", image_name = ""
Нет Description	Description = ""
Нет Specs	Specs = ""
Нет Spec_detail	Spec_detail = ""
Поврежденный HTML	Записать ошибку; пропустить конкретный товар
Дублированный product URL	Не обрабатывать повторно
Неизвестная DOM-структура	Не угадывать selector; логировать и оставить поле пустым

Количество retry не следует дублировать в новом модуле, если соответствующие timeout/network settings уже определены в config.py.
...
Поскольку сайт использует URL-пагинацию, клик по Next не нужен. В предоставленном HTML это подтверждено ссылками ?p=2, ?p=3, ?p=4, ?p=5. 

Для защиты от бесконечного цикла следует вести множество уже посещенных URL.

12. Риски

Cloudflare — основной технический риск; в cookies присутствует cf_clearance. 

Login — не требуется для предоставленных страниц.
...
Изменение DOM — реальный риск.

Specs / Spec_detail — фактическая DOM-структура отсутствует в предоставленном product HTML.

Скачивание изображений — есть архитектурный вопрос из-за неизменяемого List[str]-контракта.

Breadcrumb — пример спецификации и фактическая breadcrumb-цепочка требуют уточнения семантики.

13. Что остается неизвестным
Критично
1. DOM блока Specs / Spec_detail

Предоставленные материалы не содержат полного HTML блока характеристик. Поэтому нельзя честно указать точные selectors.

Это единственный существенный пробел для реализации parser.

2. Полный DOM Description

Сокращенный product HTML не доходит до блока полного Description.

Точный selector нельзя выдумывать.

3. Полный набор изображений

Category HTML показывает одно изображение, но спецификация предусматривает несколько. Нужно увидеть реальный image gallery product page.

4. Точный способ сохранения изображений

Нужно проверить существующий PlaywrightEngine и OUTPUT_DIR.

Если ядро не предоставляет механизма сохранения resource/image, требование «скачивать изображения» не укладывается в неизменяемый контракт двух модулей.

5. Семантика Breadcrumb
...
но пример DS-PRK-Scraper.json содержит:

Home

Нельзя самостоятельно решить, что именно клиент считает Breadcrumb.
...
Причина — parser не зависит от сети и может тестироваться на сохраненном HTML.
...
Но перед реализацией Description, Specs, Spec_detail необходимо получить полный HTML соответствующих блоков.

2. scraper.py

После определения parser-селекторов:
...
На тестовом запуске остановиться после двух товаров.
...
затем существующий вызов parser из main.py.

4. Тестирование

Минимальный набор:

category HTML → 2 корректных product URLs;

product HTML → правильный Title;

product HTML → правильные Price/Sale price;

отсутствие Sale price → пустое поле;

HTML → чистый текст без тегов;

несколько изображений сохраняют соответствующий порядок;

отсутствующие поля не вызывают exception;

итог содержит ровно 11 полей;

порядок полей соответствует DS-PRK-Scraper.json;

тестовая выборка содержит ровно 2 товара.

15. Итоговое резюме

Технология: существующий PlaywrightEngine для навигации и получения HTML + BeautifulSoup для offline parsing.
...
helpers для breadcrumb, title, descriptions, prices, images и характеристик.

Итоговая структура: ровно 11 полей:
...
Главные риски: Cloudflare, изменение DOM, отсутствующий в материалах HTML блок Specs/Spec_detail, отсутствующий полный Description, неоднозначность Breadcrumb и архитектурная проблема с физическим скачиванием изображений.

Главный вывод: для теста двух товаров архитектура в целом достаточна и не требует изменений main.py, browser.py, config.py, utils.py или exporter.py. Единственные вопросы, которые нельзя корректно «додумать», — фактическая DOM-структура характеристик/полного описания и способ локального сохранения изображений в рамках существующего PlaywrightEngine. Всё остальное можно проектировать непосредственно по имеющимся данным. 
