...
Прочитал prompt целиком, включая `DS-PRK-Scraper.json`, входные HTML-фрагменты, ограничения ядра и требования к формату ответа. Ниже — проектирование реализации **без готового Python-кода**, строго в рамках существующих интерфейсов. GitHub+1


## Почему выбран HTML + Playwright


Выбран **Playwright через существующий PlaywrightEngine + HTML/BeautifulSoup parsing**.
...
- API/GraphQL в предоставленных материалах не обнаружен.

- Нужные данные присутствуют непосредственно в HTML.

- Framework уже жестко использует `PlaywrightEngine`; заменять его на `requests`/`httpx` нельзя.

- На сайте есть признаки Cloudflare и cookies, поэтому браузерный контекст предпочтительнее прямых HTTP-запросов.

- Страницы товаров содержат дополнительные данные, которых нет в карточке листинга: breadcrumb, short description, description и характеристики.

- Pagination обычная, через `?p=N`, поэтому infinite scroll/перехват XHR не нужен.


**Плюсы:** соответствует существующей архитектуре, работает с JS/Cookies/Cloudflare-контекстом, позволяет получать полный DOM товара.


**Минусы:** медленнее прямого HTTP, возможны timeout/Cloudflare/CAPTCHA, а HTML-селекторы зависят от структуры сайта.
...
https://www.professionele-koeling.nl/koelkasten-kisten.html        ↓PlaywrightEngine        ↓scraper.scrape_data(engine)        ↓HTML category page        ↓извлечение первых 2 product URL        ↓Playwright → product page #1Playwright → product page #2        ↓HTML product pages        ↓List[str]        ↓main.py        ↓parser.parse_html_data(...)        ↓BeautifulSoup        ↓List[dict]        ↓exporter.py        ↓JSON
Для полного scrape алгоритм тот же, но scraper продолжает переходить по страницам `?p=2`, `?p=3` и т. д., пока не закончится pagination.


Ключевой момент: **scraper занимается навигацией и извлечением URL товаров, parser — исключительно преобразованием уже полученного HTML в данные.** Это соответствует зафиксированному контракту `scrape_data(engine) → List[str]`. GitHub
...
**Назначение:** выполнить весь сетевой обход и вернуть HTML страниц товаров.
...
- извлечь URL карточек;

- взять первые 2 уникальных product URL;

- открыть каждый product URL;

- получить HTML каждой product page;

- вернуть список из двух HTML.


Для полного запуска вместо ограничения `2` продолжить pagination до последней страницы.
...
**Назначение:** открыть страницу категории и получить итоговый HTML.
...
Перед получением HTML необходимо дождаться появления:


CSSul.products-grid
или более конкретного элемента:


CSSul.products-grid li.item
Фактическая структура категории подтверждена входным HTML. GitHub
...
CSSdiv.product-name h1
Дополнительно желательно дождаться:


CSSdiv.price-box
Но отсутствие одного необязательного блока не должно автоматически означать ошибку страницы.
...
Это допустимая HTML-обработка внутри scraper, поскольку она используется исключительно для **навигации**, а не для извлечения бизнес-полей.


Искомая структура:


CSSul.products-grid li.item a.product-image[href]
или, как более надежный вариант:


CSSul.products-grid li.item h2.product-name a[href]
Из каждой карточки получить абсолютный URL товара.


Например, предоставленный HTML содержит:


https://www.professionele-koeling.nl/koelkasten-kisten/polar-dm071.html
Нужно удалять дубликаты, сохраняя порядок появления.
...
Pagination уже явно присутствует.


Искомый селектор:


CSSdiv.pager div.pages li.next a[href]
В предоставленном HTML ссылка выглядит как:


...?p=2
Следовательно, для полного scrape можно последовательно использовать URL следующей страницы, пока `li.next a` существует. GitHub


**Не нужно** самостоятельно вычислять количество страниц по checkpoint.


В HTML есть актуальный для предоставленного материала индикатор:


Artikelen 1 tot 36 van 429 in totaal
и ссылки на следующие страницы. При этом старый `checkpoint.json` прямо запрещено использовать как источник актуального количества товаров. GitHub



### _download_product_images(engine, image_urls)


Это единственная дополнительная функция, связанная со скачиванием изображений.


Она относится к scraper, потому что скачивание — сетевой процесс.
...
- список оригинальных image URL.


**Выход:** локальные имена/пути сохраненных файлов.


Однако здесь есть архитектурное ограничение: `main.py` передает parser только HTML. Поэтому scraper не должен пытаться передавать отдельную структуру с путями изображений.
...
- scraper скачивает изображения как побочный сетевой результат;

- parser получает image URL из HTML;

- `image_name` определяется из basename URL;

- правила локального каталога берутся из существующего `config.py`, а не дублируются в scraper.


**ПРЕДПОЛОЖЕНИЕ:** существующий `config.py` предоставляет либо подходящий каталог для изображений, либо его можно использовать через уже существующую конфигурацию. Из prompt конкретное имя image-directory неизвестно, поэтому придумывать его нельзя.
...
category page ↓wait for product cards ↓extract product URLs ↓deduplicate ↓take first 2 ↓fetch product #1 ↓fetch product #2 ↓return [html_product_1, html_product_2]
Не следует использовать конкретно `Polar DM071` и `Polar GE579` как жестко зашитые товары. В материалах они являются примерами HTML, а тест должен брать первые два фактически найденных товара.


### Полный режим


page 1 ↓extract product URLs ↓fetch products ↓find next URL ↓page 2 ↓... ↓no next URL ↓finish
Для текущего задания `scrape_data()` должен остановиться после получения **2 product pages**, поскольку клиент сначала требует именно тестовый файл из двух товаров.
...
CSSdiv.breadcrumbsdiv.short-descriptiondiv.price-box
Для характеристик selector должен быть установлен после просмотра полного product HTML, которого в предоставленном фрагменте нет.


**ПРЕДПОЛОЖЕНИЕ:** характеристики находятся в стандартном информационном блоке Magento product page, но конкретный selector нельзя надежно назвать по имеющемуся сокращенному HTML. Его необходимо определить по фактическому полному DOM перед реализацией.
...
- между открытием category pages;

- между переходами на product pages;

- после завершения загрузки страницы перед следующим сетевым действием;

- между последовательными страницами pagination.


Не реализовывать собственную функцию задержки.
...
Parser **не выполняет никаких сетевых запросов**.


Он получает HTML и превращает его в строго заданные записи.


## 3.1. Интерфейс функций


ФункцияНазначениеВходные параметрыВозвращаемое значение`parse_html_data(html_pages)`Обработать список HTML product pages`List[str]``List[dict]``parse_product(html)`Извлечь одну полную товарную запись`str``dict``parse_breadcrumb(soup)`Извлечь BreadcrumbBeautifulSoup`str``parse_short_description(soup)`Получить чистый short descriptionBeautifulSoup`str``parse_price(soup)`Извлечь обычную ценуBeautifulSoup`number/str``parse_sale_price(soup)`Извлечь sale priceBeautifulSoup`number/str``parse_description(soup)`Извлечь описание без HTMLBeautifulSoup`str``parse_images(soup)`Извлечь image URLs и именаBeautifulSoupданные для `imageurl`/`image_name``parse_specs(soup)`Извлечь данные SpecsBeautifulSoupсоответствующее значение`parse_spec_detail(soup)`Извлечь Spec_detailBeautifulSoupсоответствующее значение
Отдельный `parse_listing()` здесь не нужен: конечные записи должны строиться из product pages, а scraper уже отбирает нужные страницы товаров.
...
Это непосредственно соответствует авторитетному `DS-PRK-Scraper.json`. GitHub
...
HTMLproduct URL
Значение — URL конкретной product page.


Важно: не использовать category URL из примера `DS-PRK-Scraper.json` как значение для каждого товара.
...
Нужно привести breadcrumb к тому представлению, которое предусмотрено финальной спецификацией.


**ПРЕДПОЛОЖЕНИЕ:** поскольку поле называется единственным `Breadcrumb`, его следует хранить как одну строку, а не добавлять отдельные колонки.
...
Polar GE579
Не использовать title из `<title>` страницы.
...
- остальные HTML tags → только текст.


Телефонный рекламный блок:


Advies nodig, of meerdere stuks...
**ПРЕДПОЛОЖЕНИЕ:** поскольку этот блок физически находится внутри `short-description`, его не следует самостоятельно удалять без явного правила клиента. Поэтому parser должен брать содержимое блока как есть после очистки HTML.
...
Нужно искать product-image elements на product page, а не использовать thumbnail из category page, если product page предоставляет оригинальные изображения.
...
url1,url2,url3
Именно такое разделение задано спецификацией: `"if multipli seperated by comma"`. GitHub
...
Для каждого скачиваемого изображения получить filename из URL.
...
Также разделять запятой.


Не добавлять отдельные поля вроде `image_path`, `image_count`, `downloaded` и т. п.
...
CSSdiv.price-box p.old-price span.price
В предоставленном HTML:


€ 255,99
Результат должен проходить через существующий `utils.clean_price()`.


Нельзя реализовывать второй механизм очистки цены.
...
CSSdiv.price-box p.special-price span.price
В примере:


€ 229,00
Если скидки нет — **пустое значение**, не копировать обычную цену. Это прямо указано в исходных ответах клиента. GitHub
...
Источник — полный блок product description.


HTML должен быть удален.


Нужно сохранить содержательный текст, включая:
...
Разделение строк нужно сохранить настолько, насколько это возможно после очистки HTML.


Не преобразовывать описание в отдельные дополнительные поля.
...
Здесь особенно важно **не придумывать собственную схему**.


Авторитетная спецификация говорит:


Specs:https://www.awesomescreenshot.com/image/62286448?key=...each own colom
и:


Spec_detail:evertything before the : is the header
При этом в prompt прямо указано, что эти значения являются **инструкциями по заполнению**, а не поврежденными данными. GitHub


Следовательно, алгоритм должен быть таким:


### Specs


Из блока характеристик получить каждую строку specification.


Не объединять разные характеристики в один произвольный текстовый блок.


### Spec_detail


Для каждой specification разделить строку по первому `:`:
...
Inhoud: 29 literEnergieklasse: FKoelmiddel: R600aGewicht: 13 kg
При отсутствии конкретной характеристики оставить значение пустым, как предписано клиентом.


**ПРЕДПОЛОЖЕНИЕ:** точный способ сериализации множества `header/value` внутри двух существующих полей не определен в доступном HTML и не должен выдумываться. Нужно сохранить установленную структуру `DS-PRK-Scraper.json`; при реализации конкретный delimiter/JSON-представление должен соответствовать тому, что ожидает exporter/тестовая fixture проекта.



# 3.3. Финальная структура записи


Для каждого товара parser должен возвращать **только эти 11 полей и именно в этом порядке**:
...
Даже если они доступны на странице, они отсутствуют в авторитетной спецификации.
...
СценарийДействиеTimeout categoryповторить ограниченное число раз; затем log + остановить текущий этапTimeout productповторить; после исчерпания retries log + пропустить конкретный товарHTTP/Browser 403записать в лог; остановить scrape, не пытаться обходить защиту агрессивными запросамиCloudflare challengeзаписать в лог; остановить выполнение для ручного вмешательстваCAPTCHAlog + остановить scraperНет product cardslog + завершить paginationНет `next`нормальное завершение paginationНет titleзаписать пустое значение и продолжить, если страница существуетНет priceпустое значение, не падатьНет sale price`""`Нет description`""`Нет images`""`Нет характеристикипустое значениеБитый HTMLBeautifulSoup пытается обработать; если критически невозможно — log + skipОшибка скачивания изображенияlog; не терять сам товар из-за одной картинкиДубликат product URLпропустить повторПустая product pagelog + skip
Ключевой принцип: **ошибка одного необязательного поля не должна ломать весь товар или весь scrape.**
...
- **Cloudflare** — существенный риск. В материалах присутствует `cf_clearance`.

- **Login** — не обнаружен.

- **Rate limiting** — потенциальный риск.

- **CAPTCHA** — возможна как часть антибот-защиты, хотя в предоставленных материалах не зафиксирована.

- **Infinite Scroll** — не обнаружен.

- **Shadow DOM** — не обнаружен.

- **iframe** — не обнаружен для требуемых данных.

- **WebSocket** — не обнаружен.

- **GraphQL API** — не обнаружен.

- **Изменение DOM** — средний/высокий риск.

- **Изображения** — необходимо отличать thumbnail category page от оригинала product page.

- **Specs** — наиболее неоднозначная часть структуры.

- **Pagination** — присутствует и должна обрабатываться динамически.

- **Cookies** — могут истечь, поэтому нельзя рассчитывать на предоставленный `cookies.json` как на вечный источник доступа.


Особенно важно: значения cookies из входного prompt не следует хардкодить в `scraper.py`. Browser/context уже является ответственностью `browser.py`.
...
Сначала реализовать parser на сохраненном HTML.


Причина: он полностью детерминирован и не зависит от сети. Можно тестировать:


HTML fixture↓BeautifulSoup↓dict
Особенно отдельно протестировать:
...
PlaywrightEngine↓category↓product URLs↓product pages
Так проще изолировать сетевые проблемы от ошибок extraction.
...
Сохраняется существующий контракт:


scrape_data(engine) → List[str]
и последующий вызов parser из `main.py`. GitHub
...
- один нормальный product HTML;

- товар без sale price;
...
- ровно два товара в тестовом режиме.


### 5. Тестовый JSON


Запустить scraper с лимитом **2 товара**, проверить:
...
- `Sale price=""`, если скидки нет;

- отсутствие HTML в text fields;

- image URLs присутствуют;

- изображения реально скачаны;

- отсутствующие значения пустые.


Только после этого переходить к полному scrape.
...
Здесь нельзя выдумывать данные.


### 1. Точный DOM-блок характеристик


Предоставленный `product-page.html` заканчивается на цене. Полного HTML блока Specs нет.


Поэтому нельзя достоверно указать конечный CSS selector для характеристик.


### 2. Точный serialization format Specs / Spec_detail


Смысл инструкции определен, но точный delimiter/внутренний формат нескольких пар `header:value` не определен в доступных материалах.


### 3. Правило хранения скачанных изображений


Известно, что изображения **надо скачивать**, но конкретный каталог и окончательный naming convention в prompt не указаны.


`config.py` менять/дублировать нельзя.


### 4. Оригинальный image selector


Category HTML содержит thumbnail:


.../cache/1/small_image/295x295/...
Но из предоставленного сокращенного product HTML нельзя достоверно установить selector оригинальной галереи.


### 5. Полный актуальный DOM сайта


Предоставлены сокращенные HTML fixtures, а не полный product page.
...
**Итоговая структура:** строго 11 полей из `DS-PRK-Scraper.json`, без добавления или переименования колонок. GitHub


**Тест:** первые 2 уникальных товара из категории → две product pages → два JSON объекта.


**Главные риски:** Cloudflare, изменение DOM, оригинальные изображения и недоопределенный serialization `Specs/Spec_detail`.


**Главное архитектурное решение:** `scraper.py` не парсит бизнес-данные и не импортирует `parser.py`; он только осуществляет навигацию, получает HTML и выполняет сетевое скачивание изображений. `parser.py` не делает ни одного сетевого запроса. `main.py`, `browser.py`, `config.py`, `utils.py` и `exporter.py` остаются без изменений. GitHub
