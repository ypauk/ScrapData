...
Если данных недостаточно для точного указания селектора, URL или параметра — **НЕ включай его в секции 2 и 3** (проектирование scraper/parser).
Вместо этого вынеси его в эту секцию с указанием, какой именно файл или данные нужны для уточнения.

Пример правильного поведения:
- Есть `category-page.html` с реальным DOM → указывай точный CSS-селектор.
- Нет HTML-образца или он неполный → пиши в "Что останется неизвестным": «Не подтверждён селектор ожидания листинга — нужен реальный HTML категориальной страницы». В секцию 2.2 ничего не ставь.
...
- **Поведение на странице:**
   - Какие селекторы необходимо дождаться перед началом парсинга? **Указывай только те, которые подтверждены в предоставленных HTML-образцах (AI_INPUT). Если образец отсутствует или неполный — оставь поле пустым и вынеси в секцию "Что останется неизвестным".**
